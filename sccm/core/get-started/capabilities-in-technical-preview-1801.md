...
<!-- 1357405 --> 단계별 배포는 여러 배포를 만들지 않고도 자동으로 소프트웨어를 조정하고 순차적으로 출시합니다. 이 Technical Preview 버전에서는 관리 콘솔의 작업 순서에 대해 단계별 배포 마법사를 완료할 수 있습니다. 그러나 배포는 만들어지지 않습니다. 
...
<!-- 1356648 --> [공동 관리](/sccm/core/clients/manage/co-management-overview) 기능을 사용하면 사용자 환경의 공동 관리에 대한 정보가 포함된 대시보드를 볼 수 있습니다. Configuration Manager 콘솔에서 **모니터링** 작업 공간으로 이동하고 **업그레이드 준비**를 확장하고 **공동 관리** 대시보드를 선택합니다. 이 대시보드에는 다음과 같은 타일이 있습니다.
- **공동 관리 디바이스**: 환경 내에서 공동 관리에 사용했던 디바이스의 비율
...
- **공동 관리 상태**: 다음 범주에서 디바이스의 성공 또는 실패를 분류합니다.
...
<!-- 1357133 --> [사용자 음성 피드백](https://configurationmanager.uservoice.com/forums/300492-ideas/suggestions/8819518-software-update-patch-tuesday-scheduling)에 따라 ADR(자동 배포 규칙) 평가가 기준일로부터 오프셋되도록 예약할 수 있습니다. 예를 들어, 매월 두 번째 화요일 후 2일째 오프셋은 목요일에 규칙을 평가합니다. 
...
<!-- 1357389 --> 새로 추가된 클래스의 경우 키가 아닌 하드웨어 인벤토리 속성에 대해 255자를 초과하는 문자열 길이를 지정할 수 있습니다.
...
<!-- 1351224 & 1355146 --> 이 Technical Preview 버전에서는 클라이언트 설정에서 소프트웨어 센터 사용자 지정이 향상되었습니다. 
...
3.  **소프트웨어 센터에서 승인되지 않은 애플리케이션 숨기기**는 소프트웨어 센터의 새로운 설정입니다. 이 옵션을 사용하면 소프트웨어 센터에서 승인이 필요한 사용자 사용 가능 애플리케이션이 숨겨집니다.<!--1355146-->
...
<!-- 1356256 --> Windows 10 1709 버전 이상이 설치된 디바이스에는 [Windows Defender Application Guard](/sccm/protect/deploy-use/create-deploy-application-guard-policy)에 대한 새로운 두 가지 호스트 상호 작용 설정이 있습니다. 
...
<!-- 1236459 --> 이제 [**스크립트 실행** 기능](/sccm/apps/deploy-use/create-deploy-scripts)을 사용하면 서명된 PowerShell 스크립트를 가져오고 실행할 수 있습니다. 
