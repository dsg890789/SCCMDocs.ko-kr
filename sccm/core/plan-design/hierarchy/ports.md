...
이 테이블에 나열된 포트 외에도 절전 모드 해제 프록시도 클라이언트 간에 ICMP 에코 요청 메시지를 사용합니다. 클라이언트는 이 통신을 사용하여 네트워크의 다른 클라이언트가 절전 모드에서 해제되었는지 여부를 확인합니다. ICMP는 Ping 명령이라고도 합니다. ICMP에는 UDP 또는 TCP 프로토콜 번호가 없으므로 다음 표에는 나열되지 않습니다. 하지만 이러한 클라이언트 컴퓨터의 호스트 기반 방화벽이나 서브넷 내에 장애가 있는 네트워크 장치에서 ICMP 트래픽을 허용해야 절전 모드 해제 프록시 통신이 성공합니다.  
