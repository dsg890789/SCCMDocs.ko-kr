...
##  <a name="bkmk_certTempl"></a> CA에서 웹 서버 인증서 템플릿 만들기  
 CA에 새 CRL을 게시한 후 다음 단계는 웹 서버 인증서 템플릿을 만드는 것입니다. 이 템플릿은 등록 지점, 등록 프록시 지점, 배포 지점 및 디바이스 관리 지점 사이트 시스템 역할을 호스트하는 서버에 대한 인증서를 발급하는 데 필요합니다. 이러한 서버는 사이트 시스템 역할과 등록된 장치 간의 신뢰할 수 있는 통신을 위한 SSL 끝점이 됩니다.    아래 단계에 따라 인증서 템플릿을 만듭니다.  
