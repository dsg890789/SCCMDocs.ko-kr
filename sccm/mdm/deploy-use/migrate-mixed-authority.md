...
Intune 독립 실행형 장치가 예상대로 작동하는지 확인한 후에는 추가 사용자 마이그레이션을 시작합니다. 테스트 사용자 집합이 있는 컬렉션을 만든 것처럼 Intune 구독과 연결된 컬렉션에서 해당 컬렉션을 마이그레이션하고 제외할 사용자가 포함된 컬렉션을 만듭니다. 자세한 내용은 [Intune 구독과 연결된 컬렉션](#collection-associated-with-your-intune-subscription)을 참조하세요.
...
-   시나리오 1: *Switch-MdmDeviceAuthority* cmdlet을 사용하여 선택한 장치를 마이그레이션하고 Azure의 Intune을 사용하여 관리할 수 있는지 확인합니다. 그런 다음, 준비가 되면 [테넌트에 대해 MDM 기관을 Intune으로 변경](migrate-change-mdm-authority.md)하여 장치의 마이그레이션을 완료합니다.  

-   시나리오 2: 테넌트에 대한 MDM 기관을 Intune으로 변경할 준비가 되면 다음 작업을 수행하여 사용자 선호도가 없는 장치를 마이그레이션합니다.  
