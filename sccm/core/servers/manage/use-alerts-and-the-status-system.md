...
3.  **상태 요약 작성기** 대화 상자에서 구성할 상태 요약 작성기를 선택한 후에 **편집** 을 클릭하여 상태 요약 작성기의 속성을 엽니다. 애플리케이션 배포 또는 애플리케이션 통계 요약 작성기를 편집하는 경우에는 5단계로 진행합니다. 구성 요소 상태를 편집하는 경우에는 6단계로 건너뜁니다. 사이트 시스템 상태 요약 작성기를 편집하는 경우에는 7단계로 건너뜁니다.  

4.  애플리케이션 배포 요약 작성기 또는 애플리케이션 통계 요약 작성기의 속성 페이지를 연 후에는 다음 단계를 사용합니다.  
...
6.  사이트 시스템 상태 요약 작성기의 속성 페이지를 연 후에는 다음 단계를 사용합니다.  
    ...
    5.  스토리지 개체를 삭제하려면 개체를 선택하고 **삭제** 단추를 클릭합니다.  
...
3.  **전자 메일 알림 구성 요소 속성** 대화 상자에서 다음 정보를 지정합니다.  
    ...
    -   **경고에 대한 이메일 알림 사용**: Configuration Manager에서 SMTP 서버를 사용하여 이메일 경고를 보낼 수 있게 하려면 이 확인란을 선택합니다.  

    -   **이메일 경고를 보낼 SMTP 서버의 FQDN 또는 IP 주소**: 이러한 경고에 사용할 이메일 서버의 FQDN(정규화된 도메인 이름) 또는 IP 주소와 SMTP 포트를 입력합니다.  

    -   **SMTP 서버 연결 계정**: Configuration Manager에서 이메일 서버에 연결하는 데 사용할 인증 방법을 지정합니다.  

    -   **이메일 경고를 보낼 발신자 주소**: 경고 이메일을 보낼 이메일 주소를 지정합니다.  

    -   **SMTP 서버 테스트**: **이메일 경고를 보낼 발신자 주소**에서 지정된 이메일 주소로 테스트 이메일을 보냅니다.  
...
3.  **새 구독** 대화 상자에서 다음 정보를 지정합니다.  
    ...
    -   **이름**: 이메일 구독을 식별할 이름을 입력합니다. 최대 255자까지 입력할 수 있습니다.  

    -   **이메일 주소**: 경고를 보내려는 이메일 주소를 입력합니다. 메일 주소가 여러 개이면 세미콜론으로 구분할 수 있습니다.  

    -   **이메일 언어**: 목록에서 이메일에 사용할 언어를 지정합니다.  
