...
아래의 제품을 *정식 라이선스* 버전의 Configuration Manager 현재 분기 버전 1902로 업그레이드할 수 있습니다.
...
- Technical Preview 분기는 정식 라이선스 설치로 업그레이드할 수 없습니다. 기술 미리 보기 버전은 이후 버전의 기술 미리 보기로만 업그레이드할 수 있습니다.  
...
- 애플리케이션 카탈로그 웹 사이트 지점 및 웹 서비스 지점
...
- 프로덕션 사이트를 Configuration Manager 현재 분기로 업그레이드  
...
소프트웨어 업데이트 지점에 NLB 클러스터를 사용하는 경우 PowerShell을 사용하여 NLB 클러스터를 제거하세요. (System Center 2012 Configuration Manager SP1부터, Configuration Manager 콘솔에 NLB 클러스터를 구성하는 옵션이 없습니다.)  
...
독립적인 필수 구성 요소 검사는 Configuration Manager의 현재 분기 및 LTSB(장기 서비스 분기) 둘 다로의 업그레이드에 대해 사이트를 평가합니다. 일부 기능은 LTSB에서 지원되지 않으므로 **ConfigMgrPrereq.log**에 다음 예와 유사한 항목이 표시될 수 있습니다.
...
- **소프트웨어 센터**  
  ...
  - **작업 정보**가 월요일부터 금요일 **오전 5:00**부터 **오후 10:00**까지의 업무 시간으로 다시 설정됩니다.  
