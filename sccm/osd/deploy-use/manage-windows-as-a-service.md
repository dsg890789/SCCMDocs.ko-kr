...
-   **서비스 계획 모니터링 타일**: 사용자가 만든 서비스 계획과 각각에 대한 규정 준수 차트를 표시합니다. 이 타일을 통해 서비스 계획 배포의 현재 상태를 빠르고 간단하게 확인할 수 있습니다. 이전 배포 링이 준수에 대한 사용자의 예상을 충족하는 경우 나중에 서비스 계획(배포 링)을 선택하고, 서비스 계획 규칙이 자동으로 트리거될 때까지 기다리는 대신 **지금 배포** 를 클릭합니다.  
...
9. 사용자 환경 페이지에서 다음 설정을 구성합니다.  
    ...
    -   **다시 시작 시 소프트웨어 업데이트 배포 재평가 동작**: 다시 시작 후 다른 업데이트 배포 평가 주기를 적용 하려면 **이 배포의 업데이트에 시스템 다시 시작이 필요한 경우 옵션을 선택 하 고 업데이트 배포 평가를 실행 합니다. 다시 시작한 후 순환**합니다. 
...
10. 배포 패키지 페이지에서 기존 배포 패키지, 배포 패키지 없음을 선택하거나 새 배포 패키지를 만들도록 다음 설정을 구성합니다.  
    ...
    1. **보내기 우선 순위**: 배포 패키지의 보내기 우선 순위를 지정합니다. Configuration Manager는 패키지를 배포 지점에 보낼 때 배포 패키지의 보내기 우선 순위를 사용합니다. 배포 패키지는 높음, 중간 또는 낮음의 우선 순위에 따라 보내집니다. 우선 순위가 서로 같은 패키지들은 만들어진 순서에 따라 보내집니다. 백로그가 없는 경우 패키지는 우선 순위에 관계없이 즉시 처리됩니다.
