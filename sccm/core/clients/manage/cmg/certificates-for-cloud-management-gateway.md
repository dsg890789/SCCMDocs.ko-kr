...
> [!NOTE]
> 1802 버전부터 CMG 서버 인증 인증서는 와일드카드를 지원합니다. 일부 인증 기관은 호스트 이름에 와일드카드 문자를 사용하는 인증서를 발행합니다. 예: `*.contoso.com` 일부 조직에서는 PKI를 간소화하고 유지 관리 비용을 절약하기 위해 와일드카드 인증서를 사용합니다.<!--491233-->  
>
...
CMG는 클라이언트 인증 인증서를 신뢰해야 합니다. 이 신뢰를 수행하려면 신뢰할 수 있는 루트 인증서 체인을 제공합니다. 두 개의 신뢰할 수 있는 루트 CA 및 네 개의 중간(하위) CA를 지정할 수 있습니다. 신뢰 체인의 모든 인증서를 추가해야 합니다. 예를 들어 중간 CA에서 클라이언트 인증 인증서를 발급하는 경우 중간 및 루트 CA 인증서를 모두 추가합니다.
