...
4. 동일한 아키텍처를 가진 부팅 이미지가 없으면 Configuration Manager는 클라이언트의 아키텍처와 호환되는 부팅 이미지를 찾습니다. 즉, 2단계에서 찾은 작업 순서 목록에서 찾습니다. 예를 들어, 64비트 BIOS/MBR 클라이언트는 32비트 및 64비트 부팅 이미지와 호환됩니다. 32비트 BIOS/MBR 클라이언트는 32비트 부팅 이미지와만 호환됩니다. UEFI 클라이언트는만 일치 하는 아키텍처와 호환 됩니다. 64 비트 UEFI 클라이언트는 64 비트 부팅 이미지와 호환 하 고는 32 비트 UEFI 클라이언트는 32 비트 부팅 이미지 하 고만 호환 됩니다.
