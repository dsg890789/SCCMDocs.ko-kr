...
- 업로드된 소프트웨어 타이틀이 공개된 후에는 지정된 애플리케이션 및 분류에 대한 정보가 System Center Online Asset Intelligence 카탈로그의 일부가 되므로 다른 카탈로그 고객에게 다운로드됩니다.  

- 소프트웨어 타이틀 원본은 Asset Intelligence 카탈로그에 기록되지 않으므로 다른 고객이 사용할 수 없습니다. 그러나 개인 정보를 포함하는 애플리케이션 타이틀을 로드하지 않는지 여전히 확인해야 합니다.  
