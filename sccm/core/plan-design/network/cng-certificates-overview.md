...
description: "Configuration Manager의 CNG 인증서에 대한 개요입니다."
ms.custom: na
ms.date: 11/20/2017
...
- IIS(인터넷 정보 서비스)의 웹 사이트에 바인딩된 CNG 인증서를 사용하여 HTTPS 모드로 설치된 경우, 응용 프로그램 카탈로그 웹 서비스, 응용 프로그램 카탈로그 웹 사이트, 등록 지점 및 등록 프록시 지점 역할이 작동하지 않습니다. 소프트웨어 센터에서는 사용자 또는 사용자 그룹 컬렉션에 배포된 응용 프로그램 및 패키지를 사용 가능한 것으로 표시하지 않습니다.

- IIS의 웹 사이트에 바인딩된 CNG 인증서를 사용하여 HTTPS 모드로 설치된 경우, 상태 마이그레이션 지점이 작동하지 않습니다.

- CNG 인증서를 사용하여 클라우드 배포 지점을 만듭니다.

- NDES 정책 모듈에서 클라이언트 인증 인증서에 대한 CNG 인증서를 사용하는 경우, NDES 정책 모듈-CRP(인증서 등록 지점) 통신이 실패합니다.

- CNG 인증서가 지정된 경우 작업 순서 미디어 만들기에서 부팅 가능한 미디어를 만드는 데 실패합니다.
