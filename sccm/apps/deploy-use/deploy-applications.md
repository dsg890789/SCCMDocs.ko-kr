...
- **소프트웨어**: 이 값은 배포할 응용 프로그램을 표시합니다. 다른 애플리케이션을 선택하려면 **찾아보기**를 클릭합니다.  
...
- **Windows Embedded 장치에 대한 쓰기 필터 처리**: 이 설정은 쓰기 필터를 사용하는 Windows Embedded 장치의 설치 동작을 제어합니다. 설치 최종 기한에 또는 유지 관리 기간 동안 변경 내용을 커밋하는 옵션을 선택합니다. 이 옵션을 선택하면 다시 시작이 필요하며 디바이스에서 변경 내용이 유지됩니다. 그렇지 않으면 애플리케이션이 임시 오버레이에 설치되고 나중에 커밋됩니다.  
...
<!--1358147--> 버전 1806부터 응용 프로그램에 대한 단계적 배포를 만듭니다. 단계적 배포에서는 사용자 지정 가능한 조건 및 그룹에 따라 소프트웨어 출시를 조정하고 순차적으로 진행할 수 있습니다. 예를 들어, 파일럿 컬렉션에 애플리케이션을 배포한 다음, 성공 조건에 따라 출시를 자동으로 계속합니다. 
