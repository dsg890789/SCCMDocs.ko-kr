...
8. **사용자 환경** 페이지에서 다음과 같은 정보를 지정한 후 **다음**을 클릭합니다.  
   ...
   - **Windows Embedded 디바이스에 대한 쓰기 필터 처리**: 이 설정은 쓰기 필터를 사용하는 Windows Embedded 디바이스에서 설치 동작을 제어합니다. 설치 최종 기한에 또는 유지 관리 기간 동안 변경 내용을 커밋하는 옵션을 선택합니다. 이 옵션을 선택하면 다시 시작이 필요하며 디바이스에서 변경 내용이 유지됩니다. 그렇지 않으면 애플리케이션이 임시 오버레이에 설치되고 나중에 커밋됩니다. 작업 순서를 Windows Embedded 디바이스에 배포할 경우 해당 디바이스가 유지 관리 기간이 구성된 컬렉션의 구성원인지 확인하세요.  
