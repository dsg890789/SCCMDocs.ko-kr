...
ms.locfileid: "75826172"
---
# <a name="use-the-hotfix-installer-to-install-updates-for-configuration-manager"></a>핫픽스 설치 관리자를 사용하여 Configuration Manager의 업데이트 설치
...
핫픽스 파일에 **.update.exe** 파일 확장명이 있는 경우 [업데이트 등록 도구를 사용하여 Configuration Manager에 핫픽스 가져오기](../../../core/servers/manage/use-the-update-registration-tool-to-import-hotfixes.md)를 참조하세요.  

> [!NOTE]  
> 이 항목에서는 Configuration Manager를 업데이트하는 핫픽스를 설치하는 방법에 대한 일반적인 지침을 제공합니다. 특정 업데이트에 대한 자세한 내용은 Microsoft 지원에서 해당 KB(기술 자료)를 참조하세요.  
...
업데이트의 범위에 해당하는 경우 마법사는 사이트 서버의 Configuration Manager 설치 폴더 아래에 일련의 폴더를 만듭니다. 폴더 구조는   
...
> [!IMPORTANT]  
> 사이트 데이터베이스를 업데이트하기 전에 사이트 데이터베이스의 백업을 만드십시오. 사이트 데이터베이스에 대한 업데이트는 제거할 수 없습니다. Configuration Manager에 대한 백업을 만드는 방법에 대한 자세한 내용은 [Configuration Manager 백업 및 복구](../../../protect/understand/backup-and-recovery.md)를 참조하세요.  
...
Configuration Manager 클라이언트에 패키지를 배포하는 방법에 대한 자세한 내용은 [패키지 및 프로그램](../../../apps/deploy-use/packages-and-programs.md)을 참조하세요.  
...
컬렉션을 만드는 방법에 대한 자세한 내용은 [컬렉션을 만드는 방법](../../../core/clients/manage/collections/create-collections.md)을 참조하세요.  
