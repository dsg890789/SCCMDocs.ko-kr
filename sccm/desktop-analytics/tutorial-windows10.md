...
4. **배포 계획 만들기** 창에서 다음 설정을 구성 합니다.  
    ...
    - **Name**: 배포 계획에 대 한 고유한 이름입니다 (예: `Windows 10 pilot`).  
...
5. **Windows 운영 체제 업그레이드** 페이지에서 다음 설정을 지정한 후 **에 다음을 선택 합니다**.  
    ...
    - **제품 키**: 설치할 OS의 Windows 제품 키를 지정합니다. 인코딩된 볼륨 라이선스 키 또는 표준 제품 키를 지정하세요. 표준 제품 키를 사용하는 경우 5자로 이루어진 각 그룹을 대시(-)로 구분 합니다. 예를 들어 다음과 같은 가치를 제공해야 합니다. *XXXXX-XXXXX-XXXXX-XXXXX-XXXXX*. 볼륨 라이선스 버전용 업그레이드의 경우에는 제품 키가 필요하지 않을 수 있습니다.  
