...
System Center Configuration Manager에서 쿼리를 만들거나 가져오려면 이 항목의 다음 섹션을 참조하세요.  
...
2.  **모니터링** 작업 영역에서 **쿼리** 를 클릭한 다음 **홈** 탭의 **만들기** 그룹에서 **쿼리 만들기**를 클릭합니다.  

3.  **쿼리 만들기 마법사** 의 **일반**탭에서 쿼리에 대한 고유한 이름과 설명(옵션)을 지정합니다.  

4.  새 쿼리의 기본으로 사용할 기존 쿼리를 가져오려면 **쿼리 문 가져오기** 를 클릭한 다음 **쿼리 찾아보기** 대화 상자에서 가져올 기존 쿼리를 선택한 후 **확인**을 클릭합니다.  

5.  **개체 형식** 목록에서 반환하려는 쿼리의 개체 형식을 선택합니다. 다음 표는 검색할 수 있는 개체 형식의 몇 가지 예제를 설명합니다.  
...
6.  *&lt;쿼리 이름\>***문 속성** 대화 상자를 열려면 **쿼리 문 편집**을 클릭합니다.  

7.  *&lt;쿼리 이름\>***문 속성** 대화 상자의 **일반** 탭에서 이 쿼리가 반환되는 특성 및 표시되는 방식을 지정합니다. 새 특성을 추가하려면 **새로 만들기** 아이콘을 클릭합니다. WQL(WMI Query Language)에서 쿼리를 직접 입력 또는 편집하려면 **쿼리 언어 표시** 를 클릭할 수도 있습니다. WMI 쿼리의 예제를 보려면 이 항목의 [Example WQL queries](#BKMK_Example) 섹션을 참조하세요.  
...
10. **확인**을 클릭하여 *&lt;쿼리 이름\>***문 속성** 대화 상자를 닫습니다.  
...
2.  **모니터링** 작업 영역에서 **쿼리** 를 클릭한 다음 **홈** 탭의 **만들기** 그룹에서 **개체 가져오기**를 클릭합니다.  

3.  **개체 가져오기 마법사** 의 **MOF 파일 이름**페이지에서 **찾아보기** 를 클릭하여 가져올 쿼리가 포함된 MOF(Managed Object Format) 파일을 선택합니다.  
...
> [!TIP]  
>  Windows Server 2008 R2를 실행하는 컴퓨터를 반환하려면 **%Workstation 6.1%** 을 **%Server 6.1%**로 변경합니다.  
...
### <a name="computers-that-are-in-a-specific-active-directory-domain-services-organizational-unit-ou"></a>특정 Active Directory Domain Services OU(조직 구성 단위)에 있는 컴퓨터  
 지정된 OU에 있는 모든 컴퓨터의 NetBIOS 이름 및 OU 이름을 반환하려면 다음 쿼리를 사용합니다. **OU Name** 텍스트를 쿼리하려는 OU의 이름으로 바꿉니다.  
