---
title: "로그 파일 | System Center Configuration Manager"
...
System Center Configuration Manager에서는 클라이언트 및 사이트 서버 구성 요소가 개별 로그 파일에 프로세스 정보를 기록합니다. Configuration Manager에서는 기본적으로 클라이언트 및 서버 구성 요소 로깅을 사용하도록 설정되어 있습니다. 이러한 로그 파일의 정보를 사용하면 Configuration Manager 계층 구조에서 발생할 수 있는 문제를 해결할 수 있습니다.  

 다음 섹션에는 여러 가지 로그 파일에 대한 자세한 정보가 나와 있습니다. 이 정보를 사용하면 작업 정보에 대한 Configuration Manager 클라이언트 및 서버 로그를 보거나 모니터링하고, 문제를 해결하는 데 도움이 될 수 있는 오류 정보를 확인할 수 있습니다.  
...
##  <a name="a-namebkmkaboutlogsa-about-configuration-manager-log-files"></a><a name="BKMK_AboutLogs"></a> Configuration Manager 로그 파일 정보  
 기본적으로 Configuration Manager의 프로세스 대부분은 해당 프로세스 전용 로그 파일에 작업 정보를 기록합니다. 이러한 로그 파일은 **.LOG** 또는 **.LO_** 확장명으로 식별됩니다. Configuration Manager는 로그가 최대 크기에 도달할 때까지 .LOG 파일에 내용을 기록합니다. 로그가 가득 차면 .LOG 파일이 이름은 같지만 확장명은 .LO_인 파일에 복사되고 프로세스 또는 구성 요소는 계속 .LOG 파일에 기록합니다. .LOG 파일이 다시 최대 크기에 도달하면 .LO_ 파일을 덮어쓰게 되고 프로세스가 반복됩니다. 일부 구성 요소는 날짜 및 시간 스탬프를 로그 파일 이름에 추가하고 .LOG 확장명을 유지하여 로그 파일 기록을 설정합니다. Linux 및 UNIX용 클라이언트에는 **.LO_** 파일의 사용 및 최대 크기가 적용되지 않습니다. Linux 및 UNIX용 클라이언트에서 로그 파일을 사용하는 방법에 대한 자세한 내용은 이 항목의 [Linux 및 UNIX용 클라이언트](#BKMK_LogFilesforLnU) 섹션에서 Linux 및 UNIX용 클라이언트의 로그 파일 관리를 참조하세요.  

 로그를 보려면 Configuration Manager 로그 뷰어 도구인 CMTrace를 사용하면 됩니다. 이 도구는 Configuration Manager 원본 미디어의 **\SMSSETUP\TOOLS** 폴더에 있습니다. CMTrace 도구는 **소프트웨어 라이브러리**에 추가된 모든 부팅 이미지에도 추가됩니다.  

###  <a name="a-namebkmklogoptionsa-configure-logging-options-by-using-the-configuration-manager-service-manager"></a><a name="BKMK_LogOptions"></a> Configuration Manager Service Manager를 사용하여 로깅 옵션 구성  
 Configuration Manager에서는 로그 파일이 저장되는 위치 및 로그 파일 크기를 변경할 수 있는 옵션을 지원합니다.  

 **Configuration Manager Service Manager** 를 사용하여 로그 파일의 크기, 로그 파일의 이름과 위치를 수정하고 여러 구성 요소가 하나의 로그 파일에 기록하도록 하려면 다음 절차를 수행하세요.  

##### <a name="to-modify-logging-for-a-component"></a>구성 요소의 로깅을 수정하려면  

1.  Configuration Manager 콘솔에서 **모니터링**을 클릭하고 **시스템 상태**를 클릭한 다음 **사이트 상태** 또는 **구성 요소 상태**를 클릭합니다.  

2.  **홈** 탭의 **구성 요소** 그룹에서 **시작** 을 클릭한 다음 **Configuration Manager Service Manager**를 선택합니다.  

3.  Configuration Manager Service Manager가 열리면 관리할 사이트에 연결합니다.  

     관리할 사이트가 표시되지 않으면 **사이트**를 클릭하고 **연결**을 클릭한 다음 올바른 사이트의 사이트 서버 이름을 입력합니다.  

4.  사이트를 확장하고 관리할 구성 요소의 위치에 따라 **구성 요소** 또는 **서버**로 이동합니다.  

5.  오른쪽 창에서 구성 요소를 하나 이상 선택합니다.  

6.  **구성 요소** 메뉴에서 **로깅**을 클릭합니다.  

7.  **Configuration Manager 구성 요소 로깅** 대화 상자에서 선택한 구성 요소에 사용할 수 있는 구성 옵션을 완료합니다.  

8.  **확인** 을 클릭하여 구성을 저장합니다.  

###  <a name="a-namebkmkloglocationa-locating-configuration-manager-logs"></a><a name="BKMK_LogLocation"></a> Configuration Manager 로그 찾기  
 기본적으로 Configuration Manager 로그 파일은 로그 파일을 만드는 프로세스 및 사이트 시스템의 구성에 따라 여러 위치에 저장됩니다. 지정된 컴퓨터의 로그 위치는 달라질 수 있으므로 특정 시나리오의 문제를 해결하려면 검색을 사용하여 Configuration Manager 컴퓨터에서 관련 로그 파일을 찾습니다.  

##  <a name="a-namebkmkclientlogsa-configuration-manager-client-logs"></a><a name="BKMK_ClientLogs"></a> Configuration Manager 클라이언트 로그  
 다음 섹션에는 클라이언트 작업 및 클라이언트 설치와 관련된 로그 파일이 나열되어 있습니다.  

###  <a name="a-namebkmkclientoplogsa-client-operations"></a><a name="BKMK_ClientOpLogs"></a> 클라이언트 작업  
 다음 표에는 Configuration Manager 클라이언트에 있는 로그 파일이 나열되어 있습니다.  
...
|CITaskMgr.log|각 응용 프로그램 및 배포 유형(예: 콘텐츠 다운로드나 설치 또는 제거 작업)에 대해 시작된 작업을 기록합니다.|  
...
|CMHttpsReadiness.log|Configuration Manager HTTPS 준비 평가 도구를 실행한 결과를 기록합니다. 이 도구는 컴퓨터에 Configuration Manager에 사용할 수 있는 PKI 클라이언트 인증 인증서가 있는지 여부를 확인합니다.|  
...
|EndpointProtectionAgent|Endpoint Protection 클라이언트의 설치 및 해당 클라이언트에 대한 맬웨어 방지 정책 적용에 대한 정보를 기록합니다.|  
|execmgr.log|클라이언트에서 실행하는 패키지 및 작업 순서에 대한 세부 정보를 기록합니다.|  
|ExpressionSolver.log|자세한 정보 또는 디버그 로깅을 사용할 때 사용되는 향상된 검색 방법에 대한 세부 정보를 기록합니다.|  
...
|LocationCache.log|클라이언트의 위치 캐시 사용량 및 유지 관리에 대한 활동을 기록합니다.|  
...
|Mifprovider.log|.MIF 파일에 대한 WMI 공급자의 활동을 기록합니다.|  
...
|PolicyPlatformClient.log|파일 공급자를 제외하고 **%Program Files%\Microsoft Policy Platform**에 있는 모든 공급자의 재구성 및 호환성 프로세스를 기록합니다.|  
|PolicySdk.log|정책 시스템 SDK 인터페이스에 대한 활동을 기록합니다.|  
|Pwrmgmt.log|절전 모드 해제 프록시 클라이언트 설정을 사용하거나 사용하지 않도록 설정하고 구성하는 데 대한 정보를 기록합니다.|  
|PwrProvider.log|WMI(Windows Management Instrumentation) 서비스에서 호스팅되는 전원 관리 공급자(PWRInvProvider)의 활동을 기록합니다. 지원되는 모든 Windows 버전에서 공급자는 하드웨어 인벤토리 중에 컴퓨터의 현재 설정을 열거하고 전원 계획 설정을 적용합니다.|  
...
|smscliui.log|제어판에서 Configuration Manager 클라이언트의 사용 현황을 기록합니다.|  
...
|SWMTRReportGen.log|계량 에이전트에서 수집한 사용량 데이터 보고서를 생성합니다. 이 데이터는 Mtrmgr.log에 기록됩니다.|  
|UserAffinity.log|사용자 장치 선호도에 대한 세부 정보를 기록합니다.|  
|VirtualApp.log|App-V 배포 유형의 평가에 관한 정보를 기록합니다.|  
|Wedmtrace.log|Windows Embedded 클라이언트의 쓰기 필터와 관련된 작업을 기록합니다.|  
|wakeprxy install.log|클라이언트가 절전 모드 해제 프록시를 사용하도록 설정하는 클라이언트 설정 옵션을 받을 경우의 설치 정보를 기록합니다.|  
|wakeprxy-uninstall.log|이전에 절전 모드 해제 프록시를 사용하도록 설정한 경우 클라이언트가 절전 모드 해제 프록시를 사용하지 않도록 설정하는 클라이언트 설정 옵션을 받을 경우 절전 모드 해제 프록시를 제거하는 데 대한 정보를 기록합니다.|  
...
|로그 이름|설명|  
|--------------|-----------------|  
|ccmsetup.log|클라이언트 설정, 클라이언트 업그레이드 및 클라이언트 제거에 대한 **ccmsetup** 작업을 기록합니다. 클라이언트 설치 문제를 해결하는 데 사용할 수 있습니다.|  
|ccmsetup ccmeval.log|클라이언트 상태 및 재구성에 대한 **ccmsetup** 작업을 기록합니다.|  
...
|로그 이름|세부 정보|  
|--------------|-------------|  
|scxcm.log|Linux 및 UNIX용 Configuration Manager 클라이언트의 핵심 서비스(ccmexec.bin)에 대한 로그 파일입니다. 이 로그 파일에는 ccmexec.bin의 설치 및 진행 중인 작업에 대한 정보가 포함됩니다.<br /><br /> 이 로그 파일은 기본적으로 **/var/opt/microsoft/scxcm.log**위치에 작성됩니다.<br /><br /> 이 로그 파일의 위치를 변경하려면 **/opt/microsoft/configmgr/etc/scxcm.conf** 를 편집하고 **경로** 필드를 변경합니다. 변경 내용을 적용하기 위해 클라이언트 컴퓨터 또는 서비스를 다시 시작하지 않아도 됩니다.<br /><br /> 로그 수준은 다음 네 가지 설정 중 하나로 설정할 수 있습니다.|  
|scxcmprovider.log|Linux 및 UNIX용 Configuration Manager 클라이언트의 CIM 서비스(omiserver.bin)에 대한 로그 파일입니다. 이 로그 파일에는 nwserver.bin의 지속적인 작업에 대한 정보가 포함됩니다.<br /><br /> 이 로그 파일은 기본적으로 **/var/opt/microsoft/configmgr/scxcmprovider.log**위치에 작성됩니다.<br /><br /> 이 로그 파일의 위치를 변경하려면 **/opt/microsoft/omi/etc/scxcmprovider.conf** 를 편집하고 **경로** 필드를 변경합니다. 변경 내용을 적용하기 위해 클라이언트 컴퓨터 또는 서비스를 다시 시작하지 않아도 됩니다.<br /><br /> 로그 수준은 다음 세 가지 설정 중 하나로 설정할 수 있습니다.|  

 **두 로그 파일은 여러 수준의 로깅을 지원합니다.**  

-   **scxcm.log** - 로그 수준을 변경하려면 **/opt/microsoft/configmgr/etc/scxcm.conf**를 편집하여 **MODULE** 태그의 각 인스턴스를 원하는 로그 수준으로 변경합니다.  
...
일반 작업 상태에서는 오류 로그 수준을 사용해야 합니다. 오류 로그 수준이 가장 적은 로그 파일을 생성합니다. 로그 수준을 오류에서 경고, 정보, 추적 등으로 올리는 경우 각 수준은 이전 로그 수준보다 더 많은 데이터를 로그 파일에 기록합니다.  

####  <a name="a-namebkmkmanagelinuxlogsa-manage-log-files-for-the-client-for-linux-and-unix-client"></a><a name="BKMK_ManageLinuxLogs"></a> Linux 및 UNIX 클라이언트의 클라이언트용 로그 파일 관리  
...
예를 들어 표준 Linux 및 UNIX 명령 **logrotate** 를 사용하여 클라이언트 로그 파일의 크기 및 회전을 관리할 수 있습니다. Linux 및 UNIX용 Configuration Manager 클라이언트는 **logrotate**가 클라이언트에 로그 회전 완료 시간에 관한 신호를 보내 클라이언트가 로그 파일에 로깅을 다시 시작할 수 있도록 하는 인터페이스를 제공합니다.  
...
|로그 이름|세부 정보|  
|--------------|-------------|  
|CCMClient-*&lt;날짜_시간>*.log|응용 프로그램 관리, 인벤토리, 오류 로그 등 Mac 클라이언트 작업과 관련된 작업을 기록합니다.<br /><br /> 이 로그 파일은 Mac 컴퓨터의 **/Library/Application Support/Microsoft/CCM/Logs** 폴더에 위치합니다.|  
...
|CCMPrefPane-*&lt;날짜_시간>*.log|일반 상태 및 오류 로그를 포함하여 Mac 컴퓨터의 Configuration Manager 기본 설정 대화 상자와 관련된 작업을 기록합니다.<br /><br /> 이 로그 파일은 Mac 컴퓨터의 **~/Library/Logs** 폴더에 있습니다.|  
 ...
 또한 사이트 시스템 서버의 SMS_DM.log 로그 파일은 모바일 장치와 Mac 컴퓨터에 설정된 관리 지점과 Mac 컴퓨터 간의 통신도 기록합니다.  
...
##  <a name="a-namebkmkserverlogsa-configuration-manager-site-server-log-files"></a><a name="BKMK_ServerLogs"></a> Configuration Manager 사이트 서버 로그 파일  
 다음 섹션에는 사이트 서버에 있는 로그 파일 또는 특정 사이트 시스템 역할과 관련된 로그 파일이 정리되어 있습니다.  

###  <a name="a-namebkmksitesiteserverloga-site-server-and-site-system-server-logs"></a><a name="BKMK_SiteSiteServerLog"></a> 사이트 서버 및 사이트 시스템 서버 로그  
 다음 표에는 Configuration Manager 사이트 서버 및 사이트 시스템 서버에 있는 로그 파일이 정리되어 있습니다.  
...
|CertMgr.log|사이트 간 통신을 위한 인증서 작업을 기록합니다.|사이트 시스템 서버|  
...
|dataldr.log|Configuration Manager 데이터베이스에서 MIF(관리 정보 형식) 파일 및 하드웨어 인벤토리 처리 작업에 대한 정보를 기록합니다.|사이트 서버|  
...
|EPCtrlMgr.log|Endpoint Protection 사이트 시스템 역할 서버에서 Configuration Manager 데이터베이스로의 맬웨어 위협 정보 동기화 관련 데이터를 기록합니다.|사이트 서버|  
...
|migmctrl.log|마이그레이션 작업, 공유된 배포 지점, 배포 지점 업그레이드 등 마이그레이션 작업 관련 정보를 기록합니다.|Configuration Manager 계층 구조의 최상위 사이트 및 각 하위 기본 사이트<br /><br /> 기본 사이트가 여러 개인 계층인 경우 중앙 관리 사이트에 만들어진 로그 파일을 사용하세요.|  
|mpcontrol.log|관리 지점의 WINS 등록을 기록합니다. 10분마다 관리 지점의 가용성을 기록합니다.|사이트 시스템 서버|  
...
###  <a name="a-namebkmksiteinstallloga-site-server-installation-log-files"></a><a name="BKMK_SiteInstallLog"></a> 사이트 서버 설치 로그 파일  
...
|smstsvc.log|연결을 시작한 서버의 컴퓨터 계정을 사용하여 네트워크 연결과 사이트 간 사용 권한을 테스트하는 데 사용되는 Windows 서비스의 설치, 사용, 제거 관련 정보가 기록됩니다.|사이트 서버 및 사이트 시스템|  

###  <a name="a-namebkmkfsploga-fallback-status-point-logs-files"></a><a name="BKMK_FSPLog"></a> 대체 상태 지점 로그 파일  
...
###  <a name="a-namebkmkmploga-management-point-logs-files"></a><a name="BKMK_MPLog"></a> 관리 지점 로그 파일  
...
|MP_Ddr.log|클라이언트에서 XML.ddr 레코드를 변환하여 사이트 서버에 복사하는 작업과 관련한 정보를 기록합니다.|사이트 시스템 서버|  
...
|MP_OOBMgr.log|클라이언트에서 OTP를 수신하는 것과 관련된 관리 지점 작업을 기록합니다.|사이트 시스템 서버|  
|MP_Policy.log|정책 통신을 기록합니다.|사이트 시스템 서버|  
|MP_Relay.log|클라이언트에서 수집한 파일의 전송을 기록합니다.|사이트 시스템 서버|  
|MP_Retry.log|하드웨어 인벤터리 재시도 프로세스를 기록합니다.|사이트 시스템 서버|  
...
|WCM.log|소프트웨어 업데이트 지점 구성과, 구독된 업데이트 범주, 분류 및 언어를 위한 WSUS(Windows Server Update Services) 서버 연결에 대한 세부 정보를 기록합니다.|WSUS(Windows Server Update Services) 서버에 연결하는 사이트 서버|  
|WSUSCtrl.log|사이트에 대한 WSUS 서버의 구성, 데이터베이스 연결 및 상태에 대한 세부 정보를 기록합니다.|사이트 시스템 서버|  
|wsyncmgr.log|소프트웨어 업데이트의 동기화 프로세스에 대한 세부 정보를 기록합니다.|사이트 시스템 서버|  
...
##  <a name="a-namebkmkfunctionlogsa-log-files-for-configuration-manager-functionality"></a><a name="BKMK_FunctionLogs"></a> Configuration Manager 기능에 대한 로그 파일  
 다음 섹션에는 Configuration Manager의 다른 기능과 관련된 로그 파일이 나열되어 있습니다.  

###  <a name="a-namebkmkappmanageloga-application-management"></a><a name="BKMK_AppManageLog"></a> 응용 프로그램 관리  
 다음 표에는 응용 프로그램 관리와 관련된 정보가 포함된 로그 파일이 나열되어 있습니다.  
...
|PrestageContent.log|원격 사전 준비된 배포 지점에서 ExtractContent.exe 도구를 사용하는 데 대한 세부 정보를 기록합니다. 이 도구는 파일로 내보낸 콘텐츠를 추출합니다.|사이트 시스템 서버|  
...
|SoftwareCenterSystemTasks.log|소프트웨어 센터 필수 구성 요소 유효성 검사 활동을 기록합니다.|클라이언트|  
...
###  <a name="a-namebkmkbnrloga-backup-and-recovery"></a><a name="BKMK_BnRLog"></a> 백업 및 복구  
 다음 표에는 사이트 다시 설정 및 SMS 공급자의 변경을 포함한 백업 및 복구 작업과 관련된 정보가 포함된 로그 파일이 나열되어 있습니다.  
...
|smssqlbkup.log|사이트 서버가 아닌 다른 서버에 SQL Server를 설치할 때 사이트 데이터베이스 백업 프로세스의 결과를 기록합니다.|사이트 데이터베이스 서버|  
...
###  <a name="a-namebkmkcertificateenrollmenta-certificate-enrollment"></a><a name="BKMK_CertificateEnrollment"></a> 인증서 등록  
 다음 표에는 네트워크 장치 등록 서비스를 실행하는 서버의 인증서 등록 지점 및 Configuration Manager 정책 모듈을 사용하는 인증서 등록과 관련된 정보가 포함된 Configuration Manager 로그 파일이 나열되어 있습니다.  
...
|로그 이름|설명|로그 파일이 있는 컴퓨터|  
|--------------|-----------------|----------------------------|  
|bgbmgr.log|클라이언트 알림 작업과 온라인 처리 및 작업 상태 파일과 관련된 사이트 서버의 활동에 대한 세부 정보를 기록합니다.|사이트 서버|  
|BGBServer.log|클라이언트 및 서버 간 통신과 클라이언트로의 작업 푸시와 같은 알림 서버의 활동을 기록합니다. 또한 사이트 서버로 전송할 온라인 및 작업 상태 파일 생성 관련 정보도 기록합니다.|관리 지점|  
|BgbSetup.log|설치 및 제거 시 알림 서버 설치 래퍼 프로세스의 활동을 기록합니다.|관리 지점|  
...
|DcmWmiProvider.log|WMI(Windows Management Instrumentation)에서 구성 항목 synclet를 읽는 작업에 대한 정보를 기록합니다.|클라이언트|  

###  <a name="a-namebkmkconsoleloga-configuration-manager-console"></a><a name="BKMK_ConsoleLog"></a> Configuration Manager 콘솔  
...
|PrestageContent.log|원격 사전 준비된 배포 지점에서 ExtractContent.exe 도구를 사용하는 데 대한 세부 정보를 기록합니다. 이 도구는 파일로 내보낸 콘텐츠를 추출합니다.|사이트 시스템 역할|  
...
###  <a name="a-namebkmkextensionsa-extensions"></a><a name="BKMK_Extensions"></a> 확장  
 다음 표에는 확장과 관련된 정보가 포함된 로그 파일이 나열되어 있습니다.  
...
###  <a name="a-namebkmkmdmloga-mobile-devices"></a><a name="BKMK_MDMLog"></a> 모바일 장치  
...
####  <a name="a-namebkmkexchsrvloga-exchange-server-connector"></a><a name="BKMK_ExchSrvLog"></a> Exchange Server 커넥터  
 다음 표에는 Exchange Server 커넥터와 관련된 정보가 포함된 로그가 나열되어 있습니다.  
...
|DmClientHealth.log|모바일 장치에 대해 사용하도록 설정된 관리 지점과 통신하는 모든 모바일 장치 기존 클라이언트의 GUID를 기록합니다.|사이트 시스템 서버|  
...
|DeployToVhd.log|VHD 만들기 및 수정 프로세스에 대한 세부 정보를 기록합니다.|Configuration Manager 콘솔을 실행하는 컴퓨터|  
|Dism.log|오프라인 지원에 대한 드라이버 설치 작업 또는 업데이트 정책 작업을 기록합니다.|사이트 시스템 서버|  
|distmgr.log|PXE에 대해 배포 지점을 사용하도록 설정하는 구성에 대한 세부 정보를 기록합니다.|사이트 시스템 서버|  
...
|mcsmgr.log|구성, 보안 모드 및 가용성의 변경 사항을 기록합니다.|사이트 시스템 서버|  
...
|OfflineServicingMgr.log|운영 체제 .wim 파일에 대한 오프라인 지원 예약 및 업데이트 적용 작업의 세부 정보를 기록합니다.|사이트 시스템 서버|  
...
|로그 이름|설명|로그 파일이 있는 컴퓨터|  
|--------------|-----------------|----------------------------|  
|hman.log|사이트 구성 변경 사항 및 Active Directory Domain Services에 사이트 정보를 게시하는 작업에 대한 정보를 기록합니다.|사이트 서버|  
|SMSProv.log|사이트 데이터베이스에 대한 WMI 공급자 액세스를 기록합니다.|SMS 공급자가 설치된 컴퓨터|  
...
###  <a name="a-namebkmksunaploga-software-updates"></a><a name="BKMK_SU_NAPLog"></a> 소프트웨어 업데이트  
 다음 표에는 소프트웨어 업데이트와 관련된 정보가 포함된 로그 파일이 나와 있습니다.  또한 일부 세부 정보는 System Center Configuration Manager에서 더 이상 사용할 수 없는 기능인 네트워크 액세스 보호와 관련이 있습니다.  

|로그 이름|설명|로그 파일이 있는 컴퓨터|  
|--------------|-----------------|----------------------------|  
|ccmcca.log|Configuration Manager NAP 정책 처리를 기반으로 한 준수 평가의 처리에 대한 세부 정보를 기록하고, 준수에 필요한 각 소프트웨어 업데이트를 위한 재구성 처리 내용을 포함합니다.|클라이언트|  
...
|SdmAgent.log|재구성 및 호환성 추적에 대한 세부 정보를 기록합니다. 그러나 소프트웨어 업데이트 로그 파일인 Updateshandler.log는 호환성에 필요한 소프트웨어 업데이트를 설치하는 작업에 대한 더 자세한 세부 정보를 제공합니다.<br /><br /> 이 로그 파일은 호환성 설정과 공유됩니다.|클라이언트|  
|ServiceWindowManager.log|유지 관리 기간의 평가에 대한 세부 정보를 기록합니다.|클라이언트|  
|smssha.log|Configuration Manager 네트워크 액세스 보호 클라이언트에 대한 기본 로그 파일입니다. 여기에는 Configuration Manager의 두 구성 요소인 LS(위치 서비스)와 CCA(구성 준수 에이전트)의 병합된 상태 설명 정보가 포함됩니다. 이 로그 파일에는 Configuration Manager 시스템 상태 에이전트와 운영 체제 NAP 에이전트 간의 상호 작용 및 Configuration Manager 시스템 상태 에이전트와 구성 준수 에이전트 및 위치 확인 서비스 간의 상호 작용에 대한 정보도 둘 다 포함됩니다. NAP 에이전트가 성공적으로 초기화되었는지 여부, 상태 설명 데이터, 상태 설명 응답에 대한 정보가 제공됩니다.|클라이언트|  
|Smsshv.log|이 로그는 시스템 상태 검사기 지점의 기본 로그 파일로, 초기화 진행률과 같은 시스템 상태 검사기 서비스의 기본 작업을 기록합니다.|사이트 시스템 서버|  
|Smsshvadcacheclient.log|Active Directory Domain Services에서 Configuration Manager 성능 상태 참조를 검색하는 작업에 대한 세부 정보를 기록합니다.|사이트 시스템 서버|  
|SmsSHVCacheStore.log|저장소에서 읽은 항목 및 로컬 캐시 저장소 파일에서 제거한 항목과 같이 Active Directory Domain Services에서 검색한 Configuration Manager NAP 성능 상태 참조를 유지하는 데 사용되는 캐시 저장소에 대한 세부 정보를 기록합니다. 캐시 저장소는 구성할 수 없습니다.|사이트 시스템 서버|  
|smsSHVQuarValidator.log|클라이언트 상태 설명 정보 및 처리 작업을 기록합니다. 전체 정보를 보려면 **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\SMSSHV\Logging\\@GLOBAL** 위치에 있는 **LogLevel** 레지스트리 키를 1에서 0으로 변경합니다.|사이트 시스템 서버|  
|smsshvregistrysettings.log|서비스를 실행하는 동안의 시스템 상태 검사기 구성 요소 구성에 대한 동적 변경 사항을 기록합니다.|사이트 시스템 서버|  
|SMSSHVSetup.log|시스템 상태 검사기 지점 설치 성공 또는 실패(실패 원인 포함) 여부를 기록합니다.|사이트 시스템 서버|  
...
|WCM.log|구독한 업데이트 범주, 분류, 언어를 위해 WSUS(Windows Server Update Services) 서버에 대한 연결과 소프트웨어 업데이트 지점 구성 정보를 기록합니다.|사이트 서버|  
|WSUSCtrl.log|사이트에 대한 WSUS 서버의 구성, 데이터베이스 연결 및 상태에 대한 세부 정보를 기록합니다.|사이트 시스템 서버|  
|wsyncmgr.log|소프트웨어 업데이트의 동기화 프로세스에 대한 세부 정보를 기록합니다.|사이트 서버|  
...
###  <a name="a-namebkmkwolloga-wake-on-lan"></a><a name="BKMK_WOLLog"></a> Wake On LAN  
 다음 표에는 Wake On LAN 사용과 관련된 정보가 있는 로그 파일이 정리되어 있습니다.  
...
###  <a name="a-namebkmkwuloga-windows-update-agent"></a><a name="BKMK_WULog"></a> Windows 업데이트 에이전트  
 다음 표에는 Windows Update 에이전트와 관련된 정보가 있는 로그 파일이 정리되어 있습니다.  

|로그 이름|설명|로그 파일이 있는 컴퓨터|  
|--------------|-----------------|----------------------------|  
|WindowsUpdate.log|Windows Update 에이전트가 WSUS 서버에 연결하여 호환성 평가를 위해 소프트웨어 업데이트를 검색하는 경우에 대한 세부 정보와 에이전트 구성 요소에 대한 업데이트가 있는지 여부에 대한 세부 정보를 기록합니다.|클라이언트|  

###  <a name="a-namebkmkwsusloga-wsus-server"></a><a name="BKMK_WSUSLog"></a> WSUS 서버  
 다음 표에는 WSUS 서버와 관련된 정보가 있는 로그 파일이 정리되어 있습니다.  

|로그 이름|설명|로그 파일이 있는 컴퓨터|  
|--------------|-----------------|----------------------------|  
|Change.log|변경된 WSUS 서버 데이터베이스 정보에 대한 세부 내용을 기록합니다.|WSUS 서버|  
