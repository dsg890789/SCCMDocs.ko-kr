...
3. **User1**의 장치는 이제 Azure portal의 Intune에서 관리 됩니다.  

4. 다른 모든 디바이스는 Configuration Manager 콘솔에서 계속 관리됩니다.  

> [!Important]  
> 독립 실행형에서 하이브리드 사용자로 이동 하는 경우 7 일 동안 정책 제거 일시 중단 됩니다.<!-- SCCMDocs issue #1066 -->
...
Intune 독립 실행형 장치가 예상대로 작동하는지 확인한 후에는 추가 사용자 마이그레이션을 시작합니다. 테스트 사용자 집합이 포함 된 컬렉션을 만든 것 처럼 사용자가 마이그레이션할를 포함 하는 컬렉션을 만듭니다. Intune 구독과 연결 된 컬렉션에서 해당 컬렉션을 제외 합니다. 자세한 내용은 참조 하세요 [테스트 사용자 그룹에 Intune 독립 실행형으로 마이그레이션](#migrate-a-test-group-of-users-to-intune-standalone)합니다.
