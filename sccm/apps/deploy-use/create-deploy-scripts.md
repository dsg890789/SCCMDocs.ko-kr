...
기본적으로 사용자는 작성한 스크립트를 승인할 수 없습니다. 스크립트는 강력하고 다양하며 잠재적인 기능을 여러 디바이스에 배포할 수 있으므로 스크립트를 작성하는 사용자와 스크립트를 승인하는 사용자 간에 역할을 분리할 수 있습니다. 이러한 역할을 통해 감독 없이 스크립트를 실행할 때도 추가적으로 보안을 강화하는 효과를 얻을 수 있습니다. 쉬운 테스트를 위해 보조 승인을 해제할 수 있습니다.
...
*(버전 1710에서 도입됨)*  
스크립트에 매개 변수를 추가하면 작업의 유연성이 향상됩니다. 최대 10 개의 매개 변수를 포함할 수 있습니다. 다음은 *문자열* 및 *정수* 데이터 형식에 대한 스크립트 매개 변수가 있는 스크립트 실행 기능의 현재 기능을 요약한 것입니다. 미리 설정된 값 목록도 사용할 수 있습니다. 스크립트에 지원되지 않는 데이터 형식이 있으면 경고가 발생합니다.

**스크립트 만들기** 대화 상자에서 **스크립트** 아래의 **스크립트 매개 변수**를 클릭합니다.

각 스크립트 매개 변수에는 세부 정보 및 유효성 검사를 추가할 수 있는 자체의 대화 상자가 있습니다. 스크립트에서 기본 매개 변수 이면 매개 변수 UI에에서 열거 됩니다 하 고 설정할 수 있습니다. 스크립트를 직접 수정 하지 마십시오는 이후 configuration Manager 기본 값을 덮어쓰지 않습니다. Ui에서 "미리 채워진 제안 된 값" 나와 있지만 Configuration Manager는 런타임 시 "default" 값에 대 한 액세스를 제공 하지 않습니다이 생각할 수 있습니다. 이 해결할 수 있습니다 올바른 기본값이 스크립트를 편집 하 여 합니다. <!--17694323-->
...
디바이스 컬렉션에서 스크립트 실행을 시작한 후에는 다음 절차에 따라 작업을 모니터링합니다. 버전 1710부터는 실행되는 스크립트를 실시간으로 모니터링할 수 있으며 지정된 스크립트 실행에 대한 보고서로 돌아갈 수도 있습니다. 일부로 스크립트 상태 데이터를 정리 합니다 [오래 된 클라이언트 작업 삭제 유지 관리 작업](../../core/servers/manage/reference-for-maintenance-tasks.md) 또는 스크립트를 삭제 합니다.<br>
...
- 1810 클라이언트는 고속 통신 채널을 통해 사이트로 80KB 미만의 출력을 반환합니다. 이러한 변경으로 인해 스크립트 또는 쿼리 출력 표시 성능이 개선됩니다.  
  ...
  - 이전 버전 1802 클라이언트 상태 메시지를 사용 하 여 계속 합니다.
...
1810 버전부터 문제 해결에 대 한 추가 로깅을 추가 되었습니다.
