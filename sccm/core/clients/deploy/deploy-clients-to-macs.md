...
>[!IMPORTANT]
>  macOS Sierra를 실행하는 디바이스에 클라이언트를 배포하려면 관리 지점 인증서의 주체 이름을 관리 지점 서버의 FQDN 등을 사용하여 올바로 구성해야 합니다.
...
7.  **등록 프로필 만들기** 대화 상자에서 이 등록 프로필의 이름을 입력한 다음 **관리 사이트 코드**를 구성합니다. Mac 컴퓨터를 관리할 관리 지점이 포함된 Configuration Manager 기본 사이트를 선택합니다.  
    ...
    > [!NOTE]  
    >  이 사이트를 선택할 수 없는 경우 사이트에서 모바일 디바이스를 지원할 관리 지점을 하나 이상 구성했는지 확인합니다.  
