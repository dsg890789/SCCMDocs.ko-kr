...
1. Configuration Manager 콘솔에서 **소프트웨어 라이브러리** 작업 영역으로 이동합니다. **애플리케이션 관리**를 확장하고 **패키지**를 선택합니다.  
...
2. 애플리케이션 만들기 마법사의 **일반 정보** 페이지에서 **디바이스에서 모든 사용자에 대해 이 애플리케이션을 프로비전**하는 옵션을 사용합니다.  

   > [!Tip]  
   > 기존 애플리케이션을 수정하는 경우 이 설정은 애플리케이션 속성의 **사용자 경험** 탭에 있습니다.  

3. 디바이스 컬렉션에 애플리케이션을 배포합니다.  

4. 다른 사용자 계정으로 대상 디바이스에 로그인하고 애플리케이션을 시작합니다.  
