...
|**스토리지 메모리 구성별 모바일 장치 수**|스토리지 메모리 구성별 모바일 장치 수|  
...
|**Exchange Server 커넥터에서 관리하는 비활성 모바일 디바이스**|지정한 기간(일) 내에 Exchange Server에 연결하지 않은 Exchange Server 커넥터에서 관리되는 모바일 장치가 표시됩니다.|  
...
|**지정된 크기의 사용 가능한 프로그램 메모리를 포함하는 모바일 디바이스**|지정된 크기의 사용 가능한 프로그램 메모리를 포함하는 모든 모바일 디바이스가 표시됩니다.|  
|**지정된 크기의 사용 가능한 이동식 스토리지 메모리를 포함하는 모바일 장치**|지정된 크기의 사용 가능한 이동식 저장소 메모리를 포함하는 모든 모바일 디바이스가 표시됩니다.|  
|**인증서 갱신 문제가 있는 모바일 디바이스**|인증서를 갱신하지 못한 등록된 모바일 디바이스가 표시됩니다. 만료 기간 전에 인증서를 갱신하지 않으면 모바일 장치가 관리되지 않습니다.|  
|**사용 가능한 프로그램 메모리가 작은 모바일 디바이스(지정된 사용 가능한 KB 미만)**|프로그램 메모리가 지정한 크기(KB)보다 작은 모바일 디바이스가 표시됩니다.|  
|**사용 가능한 이동식 스토리지 메모리가 작은 모바일 장치(지정된 사용 가능한 KB 미만)**|이동식 스토리지 메모리가 지정한 크기(KB)보다 작은 모바일 장치가 표시됩니다.|  
...
|보고서 이름|설명|  
|-----------------|-----------------|  
|**디바이스의 펌웨어 상태 세부 정보**|UEFI, SecureBoot 및 TPM 상태의 세부 정보를 표시합니다. **참고**: 버전 1810에는 이 보고서가 없습니다.<!--SCCMDocs issue #1189-->|  
...
|**자산별 애플리케이션 배포**|지정한 장치나 사용자에게 배포된 애플리케이션이 표시됩니다.|  
...
**사용자 - 디바이스 선호도** 범주에는 다음과 같은 두 개 보고서가 나열됩니다.

|보고서 이름|설명|  
|-----------------|-----------------|  
|**컬렉션별 보류 중인 사용자 디바이스 선호도 연결**|이 보고서에는 컬렉션 구성원에 대해 사용량 데이터를 기준으로 하여 보류 중인 모든 사용자 디바이스 선호도 할당이 표시됩니다.|  
|**컬렉션별 사용자 디바이스 선호도 연결**|지정한 컬렉션의 모든 사용자 디바이스 연결이 표시되며 결과가 컬렉션 유형(예: 사용자 또는 디바이스)별로 그룹화됩니다.|  
...
**가상 애플리케이션** 범주에는 다음과 같은 7개 보고서가 나열됩니다.
...
|**특정 가상 애플리케이션을 포함하는 컴퓨터**|Application Virtualization Management Sequencer를 사용하여 만든, 지정된 App-V 애플리케이션 바로 가기를 포함하는 컴퓨터의 요약이 표시됩니다.|  
|**특정 가상 애플리케이션 패키지를 포함하는 컴퓨터**|지정한 App-V 애플리케이션 패키지를 포함하는 컴퓨터의 요약이 표시됩니다.|  
|**모든 가상 애플리케이션 패키지 인스턴스 수**|검색된 App-V 애플리케이션 패키지의 수가 표시됩니다.|  
|**모든 가상 애플리케이션 인스턴스 수**|검색된 App-V 애플리케이션의 수가 표시됩니다.|  
