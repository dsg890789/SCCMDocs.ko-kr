...
|자동으로 사용자 장치 선호도를 만들려면 클라이언트에서 로그인 이벤트를 감사하도록 구성해야 합니다.|Configuration Manager는 클라이언트 컴퓨터의 로컬 보안 정책에서 다음 두 개의 설정을 읽어 자동 사용자 장치 선호도를 결정합니다.<br /><br /><ul><li> **계정 로그온 이벤트 감사**</li><li>**로그온 이벤트 감사**</li></ul> 사용자와 장치 간의 관계를 자동으로 만들려면 클라이언트 컴퓨터에 이러한 두 설정이 사용하도록 설정되어 있어야 합니다. Windows 그룹 정책을 사용하여 이러한 설정을 구성할 수 있습니다.|  
...
|**2단계:** 관리 지점에 연결하는 데 클라이언트 PKI 인증서를 사용하려면 클라이언트 컴퓨터에 클라이언트 인증 인증서를 배포합니다.|클라이언트는 클라이언트 PKI 인증서를 사용하여 응용 프로그램 카탈로그에 연결할 필요가 없지만 관리 지점에 연결해야 응용 프로그램 카탈로그를 사용할 수 있습니다. 다음 시나리오에서는 클라이언트 컴퓨터에 클라이언트 인증 인증서를 배포해야 합니다.<br /><br /><ul><li>인트라넷의 모든 관리 지점에서 HTTPS 클라이언트 연결만 허용합니다.</li><li>클라이언트가 인터넷에서 응용 프로그램 카탈로그에 연결합니다.</li></ul>|인증서 요구 사항에 대한 자세한 내용은 [PKI 인증서 요구 사항](../../core/plan-design/network/pki-certificate-requirements.md)을 참조하세요.|  
