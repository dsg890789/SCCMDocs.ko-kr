...
**아니요**를 선택하면 클라이언트가 장치를 다시 시작할 때 지워지는 임시 오버레이에 설치됩니다. 이 시나리오에서는 다른 설치에서 디바이스 변경 내용을 커밋할 때까지 Endpoint Protection 클라이언트가 완전히 설치되지 않습니다. 이 구성은 기본값입니다.  
...
**간격 설정**을 선택하여 레거시 모바일 장치가 정책을 폴링하는 기간을 분 또는 시간 단위로 지정합니다. 이러한 디바이스에는 Windows CE, Mac OS X 및 Unix 또는 Linux와 같은 플랫폼이 포함됩니다.
