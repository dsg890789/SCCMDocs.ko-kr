...
Configuration Manager는 수행하려는 여러 보고 작업에 사용할 수 있는 다양한 기본 보고서를 제공합니다. 이러한 보고서에서 SQL 문을 사용하여 보고서를 직접 작성할 수도 있습니다.   
...
|**소프트웨어 09B - 사용 빈도가 적은 소프트웨어가 설치된 컴퓨터**|지정된 기간 동안 사용자가 설치된 소프트웨어 사용하지 않은 컴퓨터가 표시됩니다. 지정되는 기간은 ‘소프트웨어 09A - 사용 빈도가 적은 소프트웨어’ 보고서에 지정된 값을 기준으로 합니다.|  
...
|**자산의 구성 기준에서 구성 항목의 호환 규칙에 대한 세부 정보**|지정한 장치나 사용자에 대해 지정된 구성 항목을 준수하는 것으로 평가된 규칙에 대한 정보가 표시됩니다.|  
|**자산의 구성 기준에서 구성 항목의 충돌하는 규칙에 대한 세부 정보**|배포된 구성 항목에서 다른 규칙과 충돌하는 규칙에 대한 정보가 표시됩니다. 배포된 동일한 구성 항목 또는 또 다른 구성 항목에 다른 규칙이 포함될 수 있습니다.|  
...
|**자산의 구성 기준에서 구성 항목의 규칙 및 오류 요약 정보**|지정된 구성 항목에 대한 규칙 준수 상태 및 설정 오류 요약 정보가 표시됩니다. 구성 항목은 장치 또는 사용자에게 배포되어야 합니다.|  
...
|보고서 이름|설명|  
|-----------------|-----------------|  
|**모든 회사 소유 모바일 장치**|모든 회사 소유 모바일 장치가 표시됩니다.|
...
|**Exchange Server 커넥터에서 관리하는 모바일 장치에 대한 기본 ActiveSync 사서함 정책의 준수 상태**|Exchange Server 커넥터에서 관리하는 모바일 장치의 기본 Exchange ActiveSync 사서함 정책과 함께 준수 상태 요약 정보가 표시됩니다.|  
...
|**상태 증명 상태별 장치 목록**|상태 증명 서비스에서 보고하는 특성이 있는 장치 목록이 표시됩니다.|
|**Microsoft Intune에 사용자별로 등록된 장치 목록**|사용자가 Microsoft Intune에 등록한 모든 장치가 표시됩니다.|  
|**특정 장치 범주의 장치 목록**|특정 장치 범주에 속한 모든 장치에 대한 정보가 표시합니다.|
...
|**Microsoft Intune에 사용자별로 등록된 장치 수**|Microsoft Intune 구독에 사용하도록 설정된 사용자가 표시됩니다. 각 사용자에게 등록된 총 장치 수도 표시됩니다.|  
|**모바일 장치에 대해 보류 중인 사용 중지 및 초기화 요청**|모바일 장치에서 보류 중인 초기화 요청이 표시됩니다.|  
...
|**Windows AutoPilot 장치 정보**|Windows AutoPilot 등록에 필요한 클라이언트 장치 정보가 표시됩니다.|
...
|**할당되었지만 특정 사이트에 설치되지 않은 컴퓨터**|지정한 사이트에 할당되었지만 해당 사이트에 보고는 하지 않는 컴퓨터 목록이 표시됩니다.|  
...
|**최근 검색되지 않은 컴퓨터(지정된 기간(일) 내)**|사이트에서 최근에 검색하지 않은 컴퓨터 목록이 표시됩니다. 사이트에서 컴퓨터를 검색한 후 경과된 일 수도 표시됩니다.|  
|**최근 인벤토리에 포함되지 않은 컴퓨터(지정된 기간(일) 내)**|사이트에서 최근에 인벤토리에 포함하지 않은 컴퓨터 목록이 표시됩니다. 클라이언트에서 컴퓨터를 마지막으로 인벤토리에 포함한 시간도 표시됩니다.|  
...
|**배포에 실패한 iOS 앱(앱이 이미 설치됨)**|선택한 iOS 앱에 대한 준수 정보가 표시됩니다. 이 자습서에서는 이 앱을 '앱 스토어의 iOS용 앱 패키지'로 배포하고, 모바일 응용 프로그램 관리 정책과 연결했습니다. 이 보고서는 사용자가 이미 수동으로 설치했기 때문에 앱이 설치하지 못한 사용자 및 장치를 표시하는 데 사용됩니다.|  
...
|**계량된 프로그램이 설치되었지만 지정된 날짜 이후 프로그램을 실행하지 않은 컴퓨터**|지정된 계량형 응용 프로그램이 있는 모든 컴퓨터를 표시하지만, 지정된 날짜 이후로 프로그램을 실행한 사용자가 없습니다.|  
...
|**계량된 모든 소프트웨어 프로그램의 총 사용량**|지정한 년/월 이내에 각 소프트웨어 계량 규칙과 일치하는 프로그램을 실행한 사용자 수가 표시됩니다. 이러한 규칙은 로컬에 설치된 소프트웨어용이거나 터미널 서비스를 사용합니다.|  
|**Windows 터미널 서버에서 계량된 모든 소프트웨어 프로그램의 총 사용량**|지정한 년/월 이내에 각 소프트웨어 계량 규칙과 일치하는 프로그램을 터미널 서비스를 사용하여 실행한 사용자의 수가 표시됩니다.|  
|**계량된 특정 소프트웨어 프로그램의 총 사용량 추세 분석**|지난 해의 각 월에 지정한 소프트웨어 계량 규칙과 일치하는 프로그램을 실행한 사용자 수가 표시됩니다. 이러한 규칙은 로컬에 설치된 소프트웨어용이거나 터미널 서비스를 사용합니다.|  
...
|**관리 2 - 필요하지만 배포되지 않은 업데이트**|클라이언트에 필요한 것으로 검색되었지만 관리자가 아직 지정된 컬렉션에 배포하지는 않은 공급업체별 소프트웨어 업데이트가 모두 표시됩니다.|  
...
|**관리 7 - 콘텐츠가 누락된 배포의 업데이트**|지정된 배포에서 일부 관련 콘텐츠가 검색되지 않은 소프트웨어 업데이트를 표시합니다. 이 상태에서는 클라이언트가 업데이트를 설치할 수 없고, 따라서 배포 규정을 100% 준수할 수 없습니다.|  
|**관리 8 - 콘텐츠가 누락된 컴퓨터(보조)**|지정된 소프트웨어 업데이트가 필요하지만 관련 콘텐츠가 아직 배포 지점에 배포되지 않은 모든 컴퓨터를 표시합니다.|  
...
**Volume Purchase Program - Apple** 범주에는 다음과 같은 한 개 보고서가 나열됩니다.
