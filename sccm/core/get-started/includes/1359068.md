...
다음 쿼리에서는 가장 최근에 사용된 응용 프로그램을 가로 막대형 차트로 렌더링합니다.
...
시간 차트를 렌더링하려면 새 **bin()** 연산자를 사용하여 시간에서 이벤트를 그룹화합니다. 다음 쿼리에서는 최근 7일 동안 장치가 시작된 시기를 보여줍니다.
...
CMPivot 창의 아래쪽에 있는 **쿼리 요약** 탭을 선택합니다. 이 상태를 통해 오프라인 상태인 클라이언트를 식별하거나 오류가 발생할 수 있는 문제를 해결하는 데 도움이 됩니다. 개수 열에서 값을 선택하여 해당 상태인 특정 장치 목록을 엽니다. 

예를 들어, 오류 상태인 장치 개수를 선택합니다. 특정 오류 메시지가 표시되고 이러한 장치 목록을 내보냅니다. 특정 cmdlet을 인식하지 못하는 오류가 발생한 경우 내보낸 장치 목록에서 컬렉션을 만들어서 Windows PowerShell 업데이트를 배포합니다.  