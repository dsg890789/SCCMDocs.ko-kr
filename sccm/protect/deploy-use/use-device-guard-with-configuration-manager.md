...
Device Guard는 소프트웨어 및 하드웨어 기반 보안 기능을 모두 포함합니다. Windows Defender 응용 프로그램 제어는 PC에서 실행할 수 있는 명시적인 소프트웨어 목록을 적용하는 소프트웨어 기반 보안 계층입니다. 자체적으로 응용 프로그램 제어에는 하드웨어 또는 펌웨어 필수 구성 요소가 없습니다. Configuration Manager와 함께 배포된 응용 프로그램 제어 정책을 사용하면 이 문서에서 설명하는 최소 Windows 버전 및 SKU 요구 사항을 충족하는 대상 컬렉션의 PC에서 정책을 사용할 수 있습니다. 선택적으로 Configuration Manager를 통해 배포된 응용 프로그램 제어 정책의 하이퍼바이저 기반 보호는 가능한 하드웨어의 그룹 정책을 통해 사용할 수 있습니다.
...
Device Guard에 대한 자세한 내용은 [Device Guard 배포 가이드](https://technet.microsoft.com/itpro/windows/keep-secure/device-guard-deployment-guide)를 참조하세요.
   ...
   > [!NOTE]
   > Windows 10 버전 1709부터 구성 가능한 코드 무결성 정책을 Windows Defender 응용 프로그램 제어라고 합니다.

## <a name="using-device-guard-with-configuration-manager"></a>Configuration Manager에서 Device Guard 사용

Configuration Manager를 사용하여 Windows Defender 응용 프로그램 제어 정책을 배포할 수 있습니다. 이 정책을 사용하면 Device Guard가 컬렉션의 PC에서 실행되는 모드를 구성할 수 있습니다. 
...
- Windows 구성 요소 업데이트 소스:
    ...
    - 선택적으로, Microsoft ISG(Intelligent Security Graph)에서 결정된 대로 평판이 좋은 소프트웨어입니다. ISG에는 Windows Defender SmartScreen 및 기타 Microsoft 서비스가 포함되어 있습니다. 신뢰할 수 있는 소프트웨어가 되도록 하려면 장치에서 Windows Defender SmartScreen 및 Windows 10 버전 1709 이상을 실행해야 합니다.
...
- 클라이언트 PC에서 정책을 처리하고 나면 Configuration Manager가 해당 클라이언트에서 관리되는 설치 프로그램으로 구성됩니다. 정책을 처리한 후 이를 통해 배포된 소프트웨어는 자동으로 신뢰됩니다. Windows Defender 응용 프로그램 제어 정책이 처리되기 전에 Configuration Manager에서 설치한 소프트웨어는 자동으로 신뢰되지 않습니다.
- Windows Defender 응용 프로그램 제어 정책을 성공적으로 처리하려면 클라이언트 PC가 도메인 컨트롤러에 연결되어 있어야 합니다.
- 배포 중에 구성할 수 있는 응용 프로그램 제어 정책의 준수는 기본적으로 매일 평가하도록 예약됩니다. 정책 처리 시 문제가 발생하면 규정 준수 평가 일정을 더 짧게 구성하면 좋을 수 있습니다(예: 1시간마다). 이 일정은 장애 발생 시 클라이언트에서 Windows Defender 응용 프로그램 제어 정책을 다시 처리하는 빈도를 나타냅니다.
...
2.  **자산 및 준수** 작업 영역에서 **Endpoint Protection**을 확장한 다음, **Windows Defender 응용 프로그램 제어**를 클릭합니다.
3.  **홈** 탭의 **만들기** 그룹에서 **응용 프로그램 제어 정책 만들기**를 클릭합니다.
4.  **응용 프로그램 제어 정책 만들기 마법사**의 **일반** 페이지에서 다음 설정을 지정합니다.
    - **이름** - 이 Windows Defender 응용 프로그램 제어 정책에 대한 고유한 이름을 입력합니다. 
    - **설명** - 필요에 따라 Configuration Manager 콘솔에서 이 정책을 식별하는 데 도움이 되는 설명을 입력합니다.
    - **모든 프로세스에 대해 이 정책을 적용할 수 있도록 장치 다시 시작 적용** - 정책이 클라이언트 PC에서 처리되면 **컴퓨터 다시 시작**에 대한 **클라이언트 설정**에 따라 클라이언트에서 다시 시작이 예약됩니다.
        - Windows 10 버전 1703 이하를 실행하는 장치는 항상 자동으로 다시 시작됩니다.
        - Windows 10 버전 1709부터는 다시 시작한 다음에야 현재 장치에서 실행 중인 응용 프로그램에 새 응용 프로그램 제어 정책이 적용됩니다. 그러나 정책이 적용된 후 시작된 응용 프로그램은 새 응용 프로그램 제어 정책을 사용합니다. 
...
5.  **응용 프로그램 제어 정책 만들기 마법사**의 **포함** 탭에서 **Intelligent Security Graph에서 신뢰하는 소프트웨어 권한 부여** 를 사용할지를 선택합니다.
6. PC의 특정 파일 또는 폴더에 대한 신뢰를 추가하려면 **추가**를 클릭합니다. **신뢰할 수 있는 파일 또는 폴더 추가** 대화 상자에서 신뢰할 로컬 파일 또는 폴더 경로를 지정할 수 있습니다. 또한 연결할 권한이 있는 원격 장치에서 파일 또는 폴더 경로를 지정할 수도 있습니다. Windows Defender 응용 프로그램 제어 정책에서 특정 파일 또는 폴더에 대한 신뢰를 추가하면 다음을 수행할 수 있습니다.
...
1.  Configuration Manager 콘솔에서 **자산 및 호환성**을 클릭합니다.
2.  **자산 및 준수** 작업 영역에서 **Endpoint Protection**을 확장한 다음, **Windows Defender 응용 프로그램 제어**를 클릭합니다.
3.  정책 목록에서 배포하려는 정책을 선택하고 **홈** 탭의 **배포** 그룹에서 **응용 프로그램 제어 정책 배포**를 클릭합니다.
4.  **응용 프로그램 제어 정책 배포** 대화 상자에서 정책을 배포하려는 컬렉션을 선택합니다. 그런 다음 클라이언트에서 정책을 평가하는 일정을 구성합니다. 마지막으로 클라이언트에는 구성된 모든 유지 관리 기간 외에도 정책을 평가할 수 있는지 여부를 선택합니다.
...
- **감사만** 또는 **적용 사용** 모드에서 정책을 배포했으며 정책을 적용하기 위해 다시 시작하지 않은 장치는 신뢰할 수 없는 소프트웨어 설치에 취약합니다.
