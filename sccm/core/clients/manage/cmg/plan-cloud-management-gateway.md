...
Configuration Manager 1902 버전부터 Azure Resource Manager가 클라우드 관리 게이트웨이의 새 인스턴스를 배포하는 유일한 메커니즘입니다. 기존 배포가 계속 작동합니다.<!-- 3605704 -->
...
1902 버전부터 CMG를 경계 그룹과 연결할 수 있습니다. 이 구성을 사용하면 클라이언트 통신 시 클라이언트가 [경계 그룹 관계](/sccm/core/servers/deploy/configure/boundary-groups)에 따라 CMG로 기본 설정되거나 대체됩니다. 이 동작은 지점과 VPN 시나리오에서 특히 유용합니다. 느리고 값비싼 WAN 링크를 사용하는 대신 Microsoft Azure의 빠른 서비스를 사용하도록 클라이언트 트래픽을 유도할 수 있습니다.<!--3640932-->
...
#### <a name="example-2-hierarchy"></a>예 2: 계층 구조

Fourth Coffee는 시애틀 본사의 온-프레미스 데이터 센터에 중앙 관리 사이트를 가지고 있습니다. 한 기본 사이트는 동일한 데이터 센터에 있고, 다른 기본 사이트는 파리의 유럽 본사에 있습니다.

- 중앙 관리 사이트에서 미국 서부 Azure 지역에 CMG 서비스를 만듭니다. 전체 계층 구조에서 예상된 로밍 클라이언트 부하에 대한 VM 수를 조정합니다.
- 시애틀 기반 기본 사이트에서 단일 CMG에 연결되는 CMG 연결 지점을 만듭니다.
- 파리 기반 기본 사이트에서 단일 CMG에 연결되는 CMG 연결 지점을 만듭니다.

클라이언트는 인터넷에 로밍할 때 미국 서부 Azure 지역의 CMG와 통신합니다. CMG는 클라이언트의 할당된 기본 사이트에서 이 통신을 CMG 연결 지점으로 전달합니다.

> [!TIP]
> 지리적 위치를 위해 여러 CMG 인스턴스를 배포할 필요는 없습니다. 구성 관리자 클라이언트는 지리적으로 멀리 떨어져 있는 경우에도 클라우드 서비스에서 발생하는 약간의 지연 시간에 영향을 받지 않습니다.
...
- 인터넷 기반 클라이언트는 Windows 업데이트에서 Microsoft 소프트웨어 업데이트 콘텐츠를 무료로 가져옵니다. Microsoft 업데이트 콘텐츠가 포함된 업데이트 패키지를 클라우드 배포 지점에 배포하지 마세요. 그렇지 않으면 스토리지 및 데이터 송신 비용이 발생할 수 있습니다.  
...
- 버전 1806부터 CMG는 클라이언트에게 콘텐츠를 제공하는 클라우드 배포 지점도 될 수 있습니다. 이 기능은 필요한 인증서 및 Azure VM 비용을 줄여줍니다. 자세한 내용은 [CMG 수정](/sccm/core/clients/manage/cmg/setup-cloud-management-gateway#modify-a-cmg)을 참조하세요.<!--1358651-->  
