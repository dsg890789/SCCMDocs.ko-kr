...
배포 지점에서 설정하는 경우 Configuration Manager에서 Windows BranchCache를 사용하고, 분산된 캐시 모드에서 사용하도록 클라이언트를 구성할 수 있습니다.
...
-   **배포 지점 구성**: 배포 지점으로 구성된 사이트 시스템 서버에 **Windows BranchCache** 기능을 추가합니다.    
...
##  <a name="bkmk_dualboot"></a> 이중 부팅 컴퓨터  
 Configuration Manager는 단일 컴퓨터에서 둘 이상의 운영 체제를 관리할 수 없습니다. 관리할 컴퓨터에 둘 이상의 운영 체제가 설치되어 있는 경우 사이트의 검색 및 클라이언트 설치 메서드를 조정하여 관리되어야 하는 운영 체제에만 Configuration Manager 클라이언트를 설치하도록 합니다.  
...
|OS 배포|OS 배포를 지원하려면 IPv4를 사용해야 합니다. |  
