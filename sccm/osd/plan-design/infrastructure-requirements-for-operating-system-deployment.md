...
description: 외부 및 제품 종속성과 운영 체제 배포에 대한 요구 사항을 알아봅니다.
ms.date: 03/22/2018
...
ms.locfileid: "32352138"
---
# <a name="infrastructure-requirements-for-os-deployment-in-system-center-configuration-manager"></a>System Center Configuration Manager의 OS 배포에 대한 인프라 요구 사항
...
-   [IT 전문가를 위한 Windows 10용 Windows ADK 시나리오](/windows/deployment/windows-adk-scenarios-for-it-pros)  

-   [Windows 10용 Windows ADK 다운로드](/windows-hardware/get-started/adk-install)  
...
-   [USMT 10에 대한 일반 마이그레이션 시나리오](/windows/deployment/usmt/usmt-common-migration-scenarios)  
...
### <a name="internet-information-services-iis-on-the-site-system-servers"></a>사이트 시스템 서버의 IIS(인터넷 정보 서비스)  
 IIS는 배포 지점, 상태 마이그레이션 지점 및 관리 지점에 필요합니다. 자세한 내용은 [사이트 및 사이트 시스템 필수 조건](../../core/plan-design/configs/site-and-site-system-prerequisites.md)을 참조하세요.  


### <a name="windows-deployment-services-wds"></a>WDS(Windows 배포 서비스)  
 WDS는 PXE 배포를 수행할 경우와 멀티캐스트를 사용하여 배포 시 대역폭을 최적화할 경우에 필요합니다. 자세한 내용은 이 아티클에서 [Windows 배포 서비스](#BKMK_WDS) 섹션을 참조하세요.  
...
### <a name="windows-device-drivers"></a>Windows 장치 드라이버  
 대상 컴퓨터에 OS를 설치할 때 Windows 장치 드라이버를 사용할 수 있습니다. 부팅 이미지에서 Windows PE를 실행할 때도 사용됩니다. 자세한 내용은 [드라이버 관리](../get-started/manage-drivers.md)를 참조하세요.  
...
### <a name="os-image"></a>OS 이미지  
 Configuration Manager의 OS 이미지는 WIM(Windows Imaging) 파일 형식으로 저장됩니다. 참조 파일 및 폴더의 압축된 컬렉션을 나타냅니다. 컴퓨터에 OS를 성공적으로 설치하고 구성하기 위해 이러한 이미지가 필요합니다. 자세한 내용은 [운영 체제 이미지 관리](../get-started/manage-operating-system-images.md)를 참조하세요.  
...
### <a name="management-point"></a>관리 지점  
 관리 지점은 클라이언트와 Configuration Manager 사이트 간에 정보를 전송합니다. 클라이언트는 관리 지점을 사용하여 OS 배포를 완료하는 작업 시퀀스를 실행합니다. 작업 순서에 대한 자세한 내용은 [작업 자동화에 대한 계획 고려 사항](planning-considerations-for-automating-tasks.md)을 참조하세요.  


### <a name="distribution-point"></a>배포 지점  
 대부분의 배포에서는 OS를 배포하는 데 사용되는 데이터(예: 이미지 또는 드라이버 패키지)를 저장하기 위해 배포 지점을 사용합니다. 일반적으로 작업 순서는 배포 지점에서 데이터를 검색하여 OS를 배포합니다. 배포 지점을 설치하고 콘텐츠를 관리하는 방법에 대한 자세한 내용은 [콘텐츠 및 콘텐츠 인프라 관리](../../core/servers/deploy/configure/manage-content-and-content-infrastructure.md)를 참조하세요.  


### <a name="pxe-enabled-distribution-point"></a>PXE 사용 배포 지점  
 PXE에서 시작한 배포를 배포하려면 배포 지점에서 클라이언트의 PXE 요청을 허용하도록 구성해야 합니다. 자세한 내용은 [배포 지점 구성](/sccm/core/servers/deploy/configure/install-and-configure-distribution-points#pxe)을 참조하세요.


### <a name="multicast-enabled-distribution-point"></a>멀티캐스트 사용 배포 지점  
 멀티캐스트를 사용하여 OS 배포를 최적화하려면 배포 지점에서 멀티캐스트를 지원하도록 구성해야 합니다. 자세한 내용은 [배포 지점 구성](/sccm/core/servers/deploy/configure/install-and-configure-distribution-points#multicast)을 참조하세요.   
...
### <a name="reporting-services-point"></a>보고 서비스 지점  
 OS 배포에 Configuration Manager 보고서를 사용하려면 보고 서비스 지점을 설치하고 구성해야 합니다. 자세한 내용은 [보고](../../core/servers/manage/reporting.md)를 참조하세요.  
...
-   **작업 순서 패키지**: 만들기, 작업 순서 미디어 만들기, 삭제, 수정, 폴더 수정, 보고서 수정, 개체 이동, 읽기, 보고서 실행, 보안 범위 설정  
 ...
 자세한 내용은 [사용자 지정 보안 역할 만들기](../../core/servers/deploy/configure/configure-role-based-administration.md#BKMK_CreateSecRole)를 참조하세요.  


### <a name="security-scopes-for-os-deployments"></a>OS 배포의 보안 범위  
 관리 사용자에게 OS 배포에서 사용되는 보안 개체(예: OS 및 부팅 이미지, 드라이버 패키지, 작업 순서 패키지 등)에 대한 액세스 권한을 제공하려면 보안 범위를 사용합니다. 자세한 내용은 [보안 범위](../../core/understand/fundamentals-of-role-based-administration.md#bkmk_PlanScope)를 참조하세요.  
...
##  <a name="BKMK_SupportedOS"></a> 지원되는 운영 체제  
 [클라이언트 및 장치에서 지원되는 운영 체제](../../core/plan-design/configs/supported-operating-systems-for-clients-and-devices.md)에 지원되는 클라이언트로 나열된 모든 Windows 운영 체제는 OS 배포에 사용할 수 있습니다.  
