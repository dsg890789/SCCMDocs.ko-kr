...
> [!TIP]  
> **클라이언트 피어 캐시** 및 **클라이언트 데이터 원본** 대시보드는 버전 1610에 [시험판 기능](/sccm/core/servers/manage/pre-release-features)으로 처음 소개되었습니다. 버전 1710 버전부터 이러한 기능은 더 이상 시험판 기능이 아닙니다. 클라이언트 피어 캐시를 사용하도록 설정해야 클라이언트 데이터 원본 대시보드가 콘솔에 표시됩니다.
