...
- **콘텐츠에 대한 대체 원본 위치를 사용하도록 허용**(사용됨 또는 사용 안 됨): 배포 지점의 **경계 그룹** 탭에서 사용할 수 있는 옵션입니다. 콘텐츠를 기본 배포 지점에서 사용할 수 없는 경우 이 옵션을 통해 클라이언트에서 대체 위치로 구성된 배포 지점을 사용할 수 있습니다.  

  - **네트워크 연결 속도에 대한 배포 동작**: 각 배포는 배포 지점에 대한 연결이 느린 경우 사용할 다음 동작 중 하나로 구성됩니다.  
  ...
  - **주문형 패키지 배포**(기본 배포 지점에 배포): 패키지 또는 애플리케이션 속성의 **배포 설정** 탭에서 **기본 배포 지점으로 이 패키지의 콘텐츠 배포** 옵션을 선택하면 이 설정을 사용할 수 있습니다. 이 옵션이 활성화된 경우 Configuration Manager에서 클라이언트가 배포 지점의 콘텐츠를 요청한 후 해당 콘텐츠가 아직 없는 기본 배포 지점에 콘텐츠를 자동으로 복사할 수 있습니다.  
