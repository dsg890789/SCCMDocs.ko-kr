...
- **클라우드 관리**: 이 서비스를 사용하면 Azure AD를 사용하여 사이트 및 클라이언트를 인증할 수 있습니다. 이 인증은 다음과 같은 다른 시나리오를 지원합니다.  
    ...
    - [Azure AD 사용자 그룹 검색 구성](/sccm/core/servers/deploy/configure/configure-discovery-methods#bkmk_azuregroupdisco)

    - 특정 [클라우드 관리 게이트웨이 시나리오](/sccm/core/clients/manage/cmg/plan-cloud-management-gateway#scenarios) 지원  

    - [앱 승인 이메일 알림](/sccm/apps/deploy-use/app-approval#bkmk_email-approve)
...
|---------|---------|---------|---------|---------|---------|
|클라우드 관리 수단<br>Azure AD 검색 | 여러 개 | 퍼블릭, 프라이빗 | ![지원됨](media/green_check.png) | ![지원됨](media/green_check.png) | 가져오기, 만들기 |
...
- **애플리케이션 이름**: 앱의 식별 이름입니다.
- **회신 URL**: 이 값은 Configuration Manager에서 사용되지는 않지만, Azure AD에 필요합니다. 이 값은 기본적으로 `https://ConfigMgrService`입니다.
...
## <a name="bkmk_renew"></a> 비밀 키 갱신

> [!Note]
> 버전 1802 및 이전 버전에서 Azure 앱의 비밀 키를 갱신하려면 앱을 다시 만들어야 합니다.

### <a name="renew-key-for-created-app"></a>만든 앱의 키 갱신

1. Configuration Manager 콘솔에서 **관리** 작업 영역으로 이동하고 **Cloud Services**를 펼친 다음 **Azure Active Directory 테넌트**를 선택합니다.

1. 세부 정보 창에서 앱에 대한 Azure AD 테넌트를 선택합니다.

1. 리본에서 **비밀 키 갱신**을 선택합니다. 앱 소유자 또는 Azure AD 관리자의 자격 증명을 입력합니다.

### <a name="renew-key-for-imported-app"></a>가져온 앱의 키 갱신

Configuration Manager에서 Azure 앱을 가져온 경우 Azure Portal를 사용하여 갱신합니다. 새 비밀 키와 만료 날짜를 확인합니다. **비밀 키 갱신** 마법사에서 이 정보를 추가합니다.  
