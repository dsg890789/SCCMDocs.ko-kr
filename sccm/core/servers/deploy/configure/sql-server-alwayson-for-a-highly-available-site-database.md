...
- 모든 구성원에는 동일한 [시딩 모드](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/automatic-seeding-secondary-replicas)가 필요합니다.<!-- SCCMDocs-pr#3899 --> Configuration Manager 설치에는 설치나 복구를 통해 데이터베이스를 만들 때 이 구성을 확인하는 필수 확인 작업이 포함됩니다.
...
원격 위치의 구성원으로 가용성 그룹을 만들어야 하는 경우 가장 짧은 네트워크 대기 시간을 토대로 우선 순위를 지정합니다. 네트워크 대기 시간이 길면 복제에 실패할 수 있습니다.<!-- SCCMDocs#1381 -->
...
#### <a name="configuration-manager-supports-only-the-site-database-in-an-availability-group"></a>Configuration Manager에서 가용성 그룹의 사이트 데이터베이스만 지원
...
버전 1906부터는 사이트에서 SQL Always On 그룹에 데이터베이스를 다시 만들 수 있습니다. 이 프로세스는 수동 및 자동 시딩 어느 쪽에서든 진행이 가능합니다.<!-- SCCMDocs-pr#3846 -->
