...
> [!NOTE]  
>  하트비트 검색이 사용하지 않도록 설정되더라도 DDR은 활성 모바일 디바이스 클라이언트에 대해 계속 만들어지고 제출됩니다. 이 동작은 **오래된 검색 데이터 삭제** 작업이 활성 모바일 디바이스에 영향을 주지 않도록 방지합니다. **오래된 검색 데이터 삭제** 작업을 통해 모바일 장치에 대한 데이터베이스 레코드가 삭제되면 장치 인증서도 해지됩니다. 이 작업은 모바일 디바이스가 관리 지점에 연결하지 못하도록 차단합니다.  
...
-   **SNMP 장치:** 네트워크 검색이 SNMP 장치를 직접 쿼리할 수 있습니다. 네트워크 검색이 디바이스를 쿼리하려면 디바이스에 로컬 SNMP 에이전트가 설치되어 있어야 합니다. 또한 SNMP 에이전트에 사용하는 커뮤니티 이름을 네트워크 검색이 사용하도록 구성합니다.  
