...
독립 실행형 미디어에는 OS와 기타 모든 필수 콘텐츠를 설치하는 단계를 자동화하는 작업 순서가 포함됩니다. 이 콘텐츠에는 부팅 이미지, 운영 체제 이미지 및 장치 드라이버가 포함됩니다. 독립 실행형 미디어에는 OS 배포에 필요한 모든 항목이 저장되기 때문에 다른 미디어 유형에 필요한 것보다 더 많은 디스크 공간이 필요합니다. 중앙 관리 사이트에서 독립 실행형 미디어를 만들 때 클라이언트가 Active Directory에서 할당된 사이트 코드를 검색합니다. 자식 사이트에 생성된 독립 실행형 미디어에서 자동으로 해당 사이트의 사이트 코드를 클라이언트에 할당합니다.  
...
- 작업 순서의 **드라이버 자동 적용** 단계. 독립 실행형 미디어는 드라이버 카탈로그의 장치 드라이버 자동 적용을 지원하지 않습니다. Windows 설치 프로그램에 지정된 드라이버 집합을 제공하려면 **드라이버 패키지 적용** 단계를 수행하세요.
...
이동식 USB 드라이브를 사용하는 경우 마법사를 실행하는 컴퓨터로 USB 드라이브를 연결합니다. 이 USB 드라이브는 Windows에서 이동식 장치로 검색되어야 합니다. 마법사는 USB 드라이브에 직접 기록하여 미디어를 만듭니다. 독립 실행형 미디어는 FAT32 파일 시스템을 사용합니다. 크기가 4GB 이상인 파일을 포함하는 USB 플래시 드라이브에 독립 실행형 미디어를 만들 수 없습니다.
...
5.  **미디어 유형** 페이지에서 미디어가 이동식 USB 드라이브인지, CD/DVD 세트인지 지정합니다.  
    ...
    -   **이동식 USB 드라이브**를 선택하는 경우 콘텐츠를 저장할 드라이브를 지정합니다.  
        ...
        - **이동식 USB 드라이브 포맷(FAT32) 및 부팅 가능하도록 구성**: 기본적으로 Configuration Manager가 USB 드라이브를 준비하도록 합니다. 많은 최신 UEFI 장치에 부팅 가능한 FAT32 파티션이 필요합니다. 그러나 이 포맷은 파일의 크기와 드라이브의 전체 용량도 제한합니다. 이동식 드라이브를 이미 포맷하고 구성한 경우에는 이 옵션을 사용하지 않도록 설정합니다. 
...
12. **사용자 지정** 페이지에서 다음 정보를 지정한 후에 **다음**을 클릭합니다.  
    ...
    -   작업 순서 이전에 실행하려는 시작 전 명령을 지정합니다. 시작 전 명령은 작업 순서가 시작되기 전에 Windows PE에서 실행되는 스크립트 또는 실행 파일입니다. 자세한 내용은 [작업 순서 미디어에 대한 시작 전 명령](../understand/prestart-commands-for-task-sequence-media.md)을 참조하세요.  

         필요에 따라 **시작 전 명령을 위한 파일 포함** 을 선택하여 시작 전 명령의 필수 파일을 포함합니다.  

        > [!TIP]  
        >  작업 순서 미디어가 생성될 때 Configuration Manager는 Configuration Manager 콘솔을 실행하는 컴퓨터의 **CreateTSMedia.log** 파일에 패키지 ID 및 시작 전 명령줄을 기록합니다. 이 출력에는 작업 순서 변수의 값이 포함됩니다. 이 로그 파일을 검토하여 작업 순서 변수의 값을 확인합니다.  
...
##  <a name="BKMK_StandAloneMediaTSExample"></a> 독립 실행형 미디어에 대한 작업 순서 예  
 독립 실행형 미디어를 사용하여 운영 체제를 배포하는 작업 순서를 만들려면 다음 표를 가이드로 사용합니다. 이 표는 작업 순서 단계의 일반적인 순서를 결정하는 데 도움이 됩니다. 또한 작업 순서 단계를 논리 그룹으로 구성하고 조직할 수 있습니다. 작업 순서를 만들면이 샘플에서 다를 수 및 더 많거나 적은 작업 순서 단계 및 그룹을 포함할 수 있습니다.  
...
|Windows 설정 캡처|이미지로 다시 설치하기 전에 이 작업 순서 단계를 따라 대상 컴퓨터에서 Windows 설정을 캡처합니다. 컴퓨터 이름, 사용자 및 조직 정보 및 표준 시간대 설정을 캡처합니다.|  
|네트워크 설정 캡처|이 작업 순서 단계를 사용 하 여 작업 순서를 수신 하는 컴퓨터에서 네트워크 설정을 캡처합니다. 컴퓨터와 정보를 설정 하는 네트워크 어댑터의 도메인 또는 작업 그룹 구성원 자격을 캡처할 수 있습니다.|  
|사용자 파일 및 설정 캡처 - **(새 작업 순서 하위 그룹)**|작업 순서 그룹 내에서 작업 순서 그룹을 만듭니다. 이 하위 그룹에는 이미지로 다시 설치하기 전에 대상 컴퓨터에서 사용자 환경 데이터를 캡처하는 단계가 포함됩니다. 추가한 초기 그룹과 마찬가지로, 이 하위 그룹은 구성 및 오류 제어 개선을 위해 유사한 작업 순서 단계를 그룹화합니다.|  
...
|운영 체제-설치 **(새 작업 순서 그룹)**|다른 작업 시퀀스 하위 그룹을 만듭니다. 이 하위 그룹에는 운영 체제를 설치하는 데 필요한 단계가 포함됩니다.|  
|Windows PE 또는 하드 디스크로 다시 부팅|이 작업 순서 단계를 사용 하 여이 작업 순서를 수신 하는 컴퓨터에 대 한 다시 시작 옵션을 지정 합니다. 이 단계에서는 설치를 계속하기 위해 컴퓨터가 다시 시작된다는 메시지가 사용자에게 표시됩니다.<br /><br /> 이 단계에서는 읽기 전용 **_SMSTSInWinPE** 작업 순서 변수입니다. 연결된 값이 **false**인 경우 작업 순서 단계가 진행됩니다.|  
|운영 체제 적용|이 작업 순서 단계를 사용 하 여 대상 컴퓨터에 운영 체제 이미지를 설치 합니다. 이 단계에서는 Configuration Manager 관련 제어 파일을 제외하고 해당 볼륨에 있는 모든 파일을 삭제합니다. 그런 다음, WIM 파일에 포함된 모든 볼륨 이미지를 해당 순차적 디스크 볼륨에 적용합니다. 지정할 수 있습니다는 **sysprep** 응답 파일을 설치에 사용할 디스크 파티션을 구성 합니다.|  
|Windows 설정 적용|대상 컴퓨터에 Windows 설정 구성 정보를 구성할 때 사용하는 작업 순서 단계입니다. 이러한 설정에는 사용자 및 조직 정보, 제품 또는 라이선스 키 정보, 표준 시간대 및 로컬 관리자 암호가 포함됩니다.|  
...
|사용자 파일 및 설정-복원 **(새 작업 순서 그룹)**|다른 작업 시퀀스 하위 그룹을 만듭니다. 이 하위 그룹에는 사용자 환경을 복원하는 데 필요한 단계가 포함됩니다.|  
|사용자 상태 복원|이 작업 순서 단계를 사용하여 USMT(사용자 환경 마이그레이션 도구)를 시작합니다. USMT는 사용자 환경 캡처 단계에서 이전에 캡처한 사용자 환경 및 설정을 대상 컴퓨터로 복원합니다.|  
