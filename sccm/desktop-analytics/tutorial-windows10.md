...
- [**전역 관리자**](/azure/active-directory/users-groups-roles/directory-assign-admin-roles#company-administrator-permissions) 권한이 있는 활성 Azure 구독  
...
5. **서버 응용 프로그램 만들기** 창에서 다음 설정을 구성 합니다.  
    ...
    **로그인**을 선택 합니다. Azure에 성공적으로 인증되면 페이지에 **Azure AD 테넌트 이름**이 참조용으로 표시됩니다.
    ...
    > [!Note]  
    > **전역 관리자 권한**으로이 단계를 완료 합니다. 이러한 자격 증명은 Configuration Manager에 의해 저장 되지 않습니다. 이 가상 사용자는 Configuration Manager에서 권한이 필요 없으며, Azure 서비스 마법사를 실행하는 계정과 동일한 계정이 아니어도 상관 없습니다.  
...
4. **배포 계획 만들기** 창에서 다음 설정을 구성 합니다.  
    ...
    - **제품 및 버전**: **Windows** 제품을 선택 하 고 권장 되는 최신 버전을 선택 합니다. 예를 들어 **Windows 10, 버전 1809 (권장)** 입니다.  

    - **장치 그룹**: Configuration Manager 탭에서 그룹을 하나 이상 선택한 다음 **대상 그룹으로 설정**을 선택 합니다. 이러한 그룹은 Configuration Manager에서 동기화 되는 컬렉션입니다.  

    - **준비 규칙**: 이러한 규칙은 업그레이드할 수 있는 장치를 확인 하는 데 도움이 됩니다. **WINDOWS OS** 를 선택 하 고 다음 설정을 구성 합니다.  

        - **내 컴퓨터가 자동으로 Windows 업데이트에서 드라이버를 가져옵니다**. 기본 설정은 **Off**이며 Configuration Manager를 사용 하 여 배포할 때 권장 됩니다.  
...
3. **데이터 원본** 페이지에서 OS 업그레이드 패키지의 설치 원본 파일에 대 한 네트워크 **경로** 를 지정 합니다. 예: `\\server\share\path`  
