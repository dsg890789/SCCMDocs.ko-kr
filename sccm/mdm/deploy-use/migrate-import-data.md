...
데이터 가져오기 도구는 어떤 방식으로든 Configuration Manager 환경을 변경 하지 않습니다. 개체를 Intune으로 가져오고 하이브리드 MDM 장치를 관리 되지 않는 상태로 유지 하는 위험 없이 모든 것이 예상 대로 작동 하는지 확인할 수 있습니다. 
...
> [!Important]  
> 배포에 대 한 컬렉션이 Azure Active Directory (Azure AD)에 복제 된 Active Directory 그룹을 기반으로 하는 경우 도구를 실행할 때 적절 한 배포를 선택 하면 도구가 마이그레이션된 개체를 그룹으로 자동으로 대상으로 지정 합니다. 더 복잡 한 컬렉션 또는 직접 멤버 자격 컬렉션을 포함 하는 경우 azure AD에서 수동으로 다시 만들고 Azure의 Intune에서 개체 할당을 수동으로 대상으로 지정 해야 합니다.  


## <a name="things-to-know-before-you-run-the-tool"></a>도구를 실행하기 전에 알아야 할 사항

- 도구를 실행 하면 Configuration Manager 환경이 변경 되지 않습니다.  

- 먼저 평가판 테 넌 트를 사용 하 여 데이터 가져오기 프로세스를 테스트 합니다. 원하는 데이터를 가져왔는지 확인 한 후에는 프로덕션 Intune 테 넌 트를 사용 하 여 동일한 프로세스를 진행할 수 있습니다.  

- 데이터 가져오기 도구는 Configuration Manager 데이터를 한 번만 가져옵니다. Configuration Manager 또는 Intune에서 개체를 추적 하지 않습니다. 그러나 이전과 동일한 컴퓨터에서 가져오기를 다시 실행 하면 이전에 가져온 개체를 알 수 있습니다. 이 도구는 개체가 여전히 Intune에 있는지 또는 개체가 변경 되었는지 여부를 알지 못합니다. 데이터를 Intune으로 두 번 이상 가져오면 중복된 개체가 될 수 있습니다.  

- 일부 프로필 설정만 가져올 수 있습니다. 예를 들어 키오스크 모드 또는 PFX 설정을 가져올 수 없습니다.  

- 선택한 플랫폼에 적용 되지 않는 설정을 가진 Configuration Manager 정책이 있는 경우이 도구는 가져오는 동안 이러한 설정을 무시할 수 있습니다. 설정이 무시되면 Intune에서 정책을 가져오고 지원할 수 있습니다.  

- 이 도구는 개체를 가져올 수 없는 이유를 제공 하려고 시도 합니다. 경우에 따라 개체를 Intune으로 가져오기 전에 Configuration Manager 콘솔로 돌아가서 문제를 해결하고, Configuration Manager 개체 검색을 다시 수행한 다음, 해당 개체를 가져올 수 있습니다. 경우에 따라 Intune에서 이러한 개체를 수동으로 다시 만들어야 할 수도 있습니다.  
...
- 이 도구를 실행한 후에 추가 수동 단계를 수행해야 할 수도 있습니다. 예를 들어 Azure AD 그룹에 대 한 앱 및 정책을 대상으로 지정 합니다.  
...
- 전역 관리자는-GlobalConsent 매개 변수를 사용 하 여 처음으로 데이터 가져오기 도구를 실행 해야 합니다. 그런 다음 전역 관리자 또는 Intune 관리자가이 도구를 실행할 수 있습니다. 
...
GitHub의 **ConfigMgrTools/Intune-데이터 가져오기** 리포지토리에서 데이터 가져오기 도구를 다운로드 합니다. 다음 절차에 따라 도구를 다운로드합니다.
...
3. 실행 파일을 저장 하 고 실행 합니다. 그런 다음 Intune Data Importer 도구의 압축을 풀 대상 폴더를 선택 합니다.  
...
2. 도구가 시작 되 면 Azure에서 전역 관리자 역할이 있는 계정을 사용 하 여 로그인 합니다.  

3. **수락** 을 선택 하 여 Microsoft Graph에서 적절 한 권한이 있는 Azure에서 응용 프로그램을 만듭니다. 데이터 가져오기 도구에는 개체를 Microsoft Intune으로 가져올 수 있는 이러한 권한이 필요합니다.  

    > [!Important]  
    > 이 메시지를 수락 하면 다음 작업을 수행할 수 있는 권한을 도구에 부여 합니다.  
    > - 모든 그룹 읽기  
    > - 로그인 및 사용자 프로필 읽기  
...
> [!Note]  
> 전역 관리자가 먼저 동의를 수락 하지 않은 경우이 도구는 Intune 관리자에 대해 다음 오류를 표시할 수 있습니다. **이 응용 프로그램에 액세스할 수 없습니다**.  


### <a name="manually-map-collections-to-azure-ad-groups"></a>Azure AD 그룹에 컬렉션을 수동으로 매핑

데이터 가져오기 도구를 실행 하면 단일 Active Directory 그룹을 대상으로 하는 단일 규칙을 사용 하 여 컬렉션에서 Active Directory 그룹 이름을 추출 합니다. Intune에서 할당을 만들 때 데이터 가져오기는 Active Directory 그룹과 동일한 이름의 Azure AD 그룹을 찾습니다. 존재 하는 경우이 도구는 가져온 개체를 해당 Azure AD 그룹에 할당 합니다. 

데이터 가져오기에서 컬렉션에 대해 찾은 Active Directory 그룹 이름을 재정의 하 고 해당 컬렉션에 사용할 하나 이상의 Azure AD 그룹을 제공할 수 있습니다. 컬렉션 매핑 파일을 사용하면 통상 데이터 가져오기 도구로는 가져올 수 없는 컬렉션을 Azure AD 그룹에 매핑할 수 있습니다. 
...
컬렉션 매핑 파일을 사용하려면 **-CollectionMappingFile** 명령줄 매개 변수와 만든 컬렉션 매핑 .csv 파일의 전체 경로를 사용하여 데이터 가져오기 도구를 시작해야 합니다. 예를 들면

```IntuneDataImporter.exe -CollectionMappingFile c:\Users\myuser\Documents\collectionmapping.csv```

> [!Note]  
> 데이터 가져오기 도구는 모든 마법사 페이지에서 컬렉션 매핑 파일이 로드 되었음을 나타내는 항목을 표시 하지 않습니다. 하지만 .csv 파일을 읽은 중 발생한 오류는 모두 표시합니다. 
> 
...
1. 도구를 열고 **시작**을 선택 합니다.  

2. 정보를 읽은 후 **다음**을 선택 합니다.  

3. 이전에 내보낸 데이터 집합을 가져올지 여부를 선택하거나 가져올 개체 형식을 선택합니다.  

    - **이전 Intune Data Importer 실행에서 내보낸 데이터 집합 가져오기**: **내보낸 데이터 폴더** **찾아보기** 를 선택 합니다. 데이터 가져오기 도구를 사용 하 여 이전에 내보낸 데이터 집합을 선택 합니다. 데이터 집합을 가져오는 사용자는 데이터를 내보낸 사용자와 동일해야 합니다. 데이터를 가져온 후에는 마법사의 **요약** 페이지에 개체에 대한 요약이 표시됩니다. 요약이 맞는 것 같으면 [3단계: Intune으로 선택한 개체 가져오기](#phase-3-import-selected-objects-to-intune)를 건너뜁니다.  

        > [!Note]  
        > 사이트에서 가져올 개체를 검색 및 선택한 후 마법사의 **Intune에 로그인** 페이지에서 개체를 데이터 집합으로 내보낼 수 있습니다. 그런 다음이 페이지에서 데이터 집합을 가져올 수 있습니다. 데이터 집합은 로그인 한 사용자의 Windows 사용자 자격 증명을 사용 하 여 암호화 되므로 데이터 집합을 내보낸 사용자만 도구에서 데이터 집합을 가져올 수 있습니다.  

    - **가져올 개체 유형 선택**: 가져올 사이트 및 개체에 대 한 다음 정보를 제공 합니다.  
...
4. 사이트에서 개체 검색을 시작 하려면 **다음** 을 선택 합니다. 이 도구에는 각 개체 형식에 대한 진행률이 표시됩니다.  

    - 도구에서 선택한 개체 형식에 대한 데이터를 검색하지 못하면 해당 개체 형식에 수행된 진행률 표시줄이 즉시 표시됩니다.  

    - 선택한 개체가 데이터 **수집** 페이지에 표시 되지 않습니다.  

    - 수집 되지 않았거나 수집 프로세스 중에 취소 한 개체에 대해 도구를 다시 실행할 수 있습니다.  


### <a name="phase-2-resolve-issues-and-select-the-objects-to-import"></a>2단계: 문제 해결 및 가져올 개체 선택  

2단계에서는 도구에서 찾은 개체를 검토하고, 개체가 Intune으로 가져오지 못하게 하는 문제를 해결하고, 가져올 개체를 선택합니다. 문제를 해결 하는 경우 마법사의 **검색 환경** 페이지로 돌아가 개체를 다시 검색 합니다. 
...
1. **다음**을 클릭하여 수집된 개체를 검토합니다. 항목 선택 페이지는 수집된 각 개체 형식에 대해 사용할 수 있습니다.  
   ...
   -->
2. 각 항목 선택 페이지에서 **가져올** 열을 기준으로 개체를 정렬 하 고 가져올 없는 개체를 검토 합니다. **참고** 열의 정보는 도구에서 개체를 가져올 수 없는 이유에 대 한 세부 정보를 제공 합니다.  

3. 가져올 수 없는 개체에 대 한 문제를 해결할 지 여부를 결정 합니다. 하나 이상의 문제를 해결 하는 경우 Configuration Manager에서 데이터 선택 페이지에 도달할 때까지 **이전** 을 선택 합니다. 그런 다음 데이터를 다시 수집 하 여 문제가 해결 되었는지 확인 합니다. 가져올 수 있는 개체에 만족할 때까지 문제를 계속 해결할 수 있습니다.  
...
4. 각 항목 선택 페이지에서 가져올 개체를 선택합니다. 다음 열이 나열됩니다.  
    ...
    - **Is superseded**(대체됨)(앱별): 앱이 다른 앱으로 대체되었는지 여부를 지정합니다. 교체 된 앱을 표시 하려면 페이지 맨 위에 있는 **대체 된 앱 표시** 옵션을 선택 합니다.  
...
5. 가져올 개체를 선택 하면 요약 페이지에 나열 됩니다. 다음과 같은 작업을 수행할 수 있습니다.  

    - **세부 정보 내보내기**: 화면에 표시된 정보가 포함된 .csv 파일을 만듭니다. 또한 가져올 수 없는 개체와 가져올 수 없는 이유를 보여 줍니다. 이 파일을 레코드로 유지할 수 있습니다.  
...
1. 다음 **을 선택**하 고 다음 옵션 중 하나를 선택 합니다.  

    - **내보내기**: 사이트에서 가져올 개체를 검색 및 선택한 후 개체를 데이터 집합으로 내보낼 수 있습니다. 이렇게 하면 인터넷에 액세스할 수 없는 컴퓨터에서 개체를 검색 하 고, 데이터를 내보낸 다음, 인터넷에 액세스할 수 있는 컴퓨터에서 데이터를 가져올 수 있습니다. 데이터 집합은 로그인 한 사용자의 Windows 사용자 자격 증명을 사용 하 여 암호화 되므로 데이터 집합을 내보낸 사용자만 도구에서 데이터 집합을 가져올 수 있습니다. 이 옵션을 선택하는 경우 내보낸 데이터의 경로를 선택합니다.  

        1. **Intune에 로그인** 페이지에서 **내보내기** 를 선택 합니다.  

        2. **찾아보기** 를 선택 하 여 내보낼 대상 폴더를 선택 합니다. 이 폴더는 비어 있어야 합니다.  

        3. **시작** 을 선택 하 여 데이터를 내보냅니다. 내보내기가 완료 되 면 **닫기** 를 선택 하 여 마법사를 완료 하 고 데이터 가져오기를 닫습니다.  

        4. 동일한 자격 증명을 사용 하 여 인터넷에 연결 된 다른 컴퓨터에서 데이터 가져오기를 시작 합니다. 마법사의 두 번째 페이지에서 **이전에 내보낸 데이터 집합 가져오기** 를 선택 합니다. 데이터를 가져오면 마법사의 **Intune에 로그인** 페이지로 이동됩니다.  
...
데이터를 Intune으로 가져온 후에는 [intune에서 사용자 마이그레이션을 준비](migrate-prepare-intune.md)합니다. 
