...
- CMG 서버 인증 인증서입니다. 이 인증서는 공용 및 전역으로 신뢰할 수 있는 인증서 공급자의 SSL 인증서입니다. 예를 들어 DigiCert, VeriSign 또는 Thawte가 있지만, 이에 국한되지는 않습니다. 이 인증서를 개인 키를 사용하여 .PFX 파일로 내보냅니다.  
...
2. **리소스 만들기**를 선택하고, **계산** 범주를 선택한 다음, **클라우드 서비스**를 선택합니다. 클라우드 서비스(클래식) 블레이드를 엽니다.
...
기본 사이트 서버를 사용하여 CSR(인증서 서명 요청)을 생성하는 것이 좋습니다. 인증서를 가져올 때 CSR을 생성한 동일한 서버에 인증서를 등록해야 합니다. 그러지 않으면 필수 항목인 인증서 개인 키를 내보낼 수 없습니다.  
...
3. 인증서 내보내기 마법사에서 **다음**, **예, 개인 키를 내보냅니다.** 를 선택한 후 **다음**을 선택합니다.  
