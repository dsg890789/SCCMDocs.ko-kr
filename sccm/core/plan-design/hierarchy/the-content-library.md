...
3. 리본에서 **콘텐츠 라이브러리 관리**를 선택합니다.  

4. 콘텐츠 라이브러리 관리 창에서 **현재 위치** 필드에 로컬 드라이브 및 경로가 표시됩니다. **새 위치**에 대한 유효한 네트워크 경로를 입력합니다. 이 경로는 사이트에서 콘텐츠 라이브러리를 이동하는 위치입니다. 예를 들어 `\\server\share\folder`와 같이 이미 공유에 존재하는 폴더 이름을 포함해야 합니다. **확인**을 선택합니다.  
