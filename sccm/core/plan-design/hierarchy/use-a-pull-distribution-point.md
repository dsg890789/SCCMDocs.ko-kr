...
-   패키지 전송 관리자가 콘텐츠를 사용할 수 있음을 확인하면 콘텐츠를 다운로드할 수 있도록 풀(pull) 배포 지점에 알립니다. 이 알림이 실패하면 풀(pull) 배포 지점에 대한 소프트웨어 배포 구성 요소 **다시 시도 설정**을 기반으로 다시 시도합니다.  풀(pull) 배포 지점은 이 알림을 받으면 원본 배포 지점으로부터 콘텐츠를 다운로드합니다.  
...
## <a name="to-configure-software-distribution-component-for-pull-distribution-points"></a>풀(pull) 배포 지점에 대한 소프트웨어 배포 구성 요소를 구성하려면

1.  Configuration Manager 콘솔에서 **관리** > **사이트**를 선택합니다.  

2.  원하는 사이트를 선택하고 **사이트 구성 요소 구성** > **소프트웨어 배포**를 선택합니다.

3. **풀(pull) 배포 지점** 탭을 선택합니다.  

4.  **다시 시도 설정** 목록에서 다음 값을 구성합니다.  

    -   **재시도 횟수** - 패키지 전송 관리자가 콘텐츠를 다운로드하기 위해 풀(pull) 배포 지점에 알리려고 시도하는 횟수입니다.  이 횟수가 초과되면 패키지 전송 관리자가 전송을 취소합니다.

    -   **다시 시도하기 전 지연(분)** - 패키지 전송 관리자가 시도 간에 대기하는 시간(분)입니다. 

5.  **상태 폴링 설정** 목록에서 다음 값을 구성합니다.  

    -   **폴링 횟수** - 패키지 전송 관리자가 작업 상태를 검색하기 위해 풀(pull) 배포 지점에 연결하는 횟수입니다.  작업을 완료하기 전에 이 횟수가 초과될 경우 패키지 전송 관리자는 전송을 취소합니다.

    -   **다시 시도하기 전 지연(분)** - 패키지 전송 관리자가 시도 간에 대기하는 시간(분)입니다. 
    
    > [!NOTE]  
    >  상태 폴링 재시도 횟수가 초과되어 전송 관리자가 작업을 취소하면 풀(pull) 배포 지점에서 콘텐츠를 계속 다운로드합니다.  완료되면 적절한 상태 메시지가 패키지 전송 관리자로 전송되고 콘솔에 새 상태가 반영됩니다.
...
-   콘텐츠 전송 시 **풀(pull) 배포 지점은 일정 또는 속도 제한 구성을 사용하지 않습니다**. 이전에 설치된 배포 지점을 풀(pull) 배포 지점으로 구성할 경우 일정 및 속도 제한에 대한 구성이 저장되지만 사용되지는 않습니다. 이후에 풀(pull) 배포 지점 구성을 제거하면 일정 및 속도 제한 구성이 이전에 구성된 것으로 구현됩니다.  

    > [!NOTE]  
    >  배포 지점이 풀(pull) 배포 지점으로 구성된 경우 배포 지점의 속성에 **일정** 및 **속도 제한** 탭이 나타나지 않습니다.  

-   풀(pull) 배포 지점은 **소프트웨어 배포 구성 요소 속성**의 **일반** 탭에서 각 사이트에 대한 설정을 사용하지 않습니다.  여기에는 **동시 배포** 및 **멀티캐스트 재시도** 설정이 포함됩니다.  **풀(pull) 배포 지점** 탭을 사용하여 풀(pull) 배포 지점에 대한 설정을 구성합니다.
