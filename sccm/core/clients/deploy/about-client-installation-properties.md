...
CCMSetup.exe 명령을 사용하여 Configuration Manager 클라이언트를 설치합니다. 명령줄에 이러한 클라이언트 설치 속성을 제공하면 설치 동작을 수정합니다.
...
> [!NOTE]  
>  명령줄에서 **/source** 속성을 한 번 이상 사용하여 대체 다운로드 위치를 지정할 수 있습니다.  
...
**/mp** 명령줄 속성을 사용하여 1 초과 관리 지점을 지정합니다. 컴퓨터가 첫 번째 관리지점에 연결하지 못한 경우 지정된 목록에서 다음을 시도합니다. 여러 관리 지점을 지정할 때는 세미콜론으로 값을 구분합니다.

클라이언트가 HTTPS를 사용하여 관리 지점에 연결하는 경우 보통은 컴퓨터 이름이 아니라 FQDN을 지정해야 합니다. 값은 관리 지점의 PKI 인증서 주체 또는 주체 대체 이름과 일치해야 합니다. Configuration Manager에서는 인트라넷상의 연결에 대해 인증서에 컴퓨터 이름을 사용하도록 지원하지만 권장하는 보안 모범 사례는 FQDN입니다.

컴퓨터 이름을 사용하는 경우의 예: `ccmsetup.exe /mp:SMSMP01`  

FQDN을 사용하는 경우의 예: `ccmsetup.exe /mp:smsmp01.contoso.com`  

이 속성은 클라우드 관리 게이트웨이의 URL을 지정할 수 있습니다. 인터넷 기반 장치에서 클라이언트를 설치하려면 이 URL을 사용합니다. 이 속성에 대한 값을 가져오려면 다음 단계를 사용합니다.
- 클라우드 관리 게이트웨이를 만듭니다.
...
CCMSetup이 기본값인 서비스로 실행되지 않도록 합니다. CCMSetup이 서비스로서 실행되는 경우 컴퓨터의 로컬 시스템 계정의 컨텍스트에서 실행됩니다. 이 계정은 설치에 필요한 네트워크 리소스에 액세스할 권한이 충분하지 않을 수 있습니다. **/noservice**를 사용하면 CCMSetup.exe가 설치 시작에 사용하는 사용자 계정의 컨텍스트에서 실행됩니다. 또한 **/service** 속성으로 CCMSetup.exe를 실행하는 스크립트를 사용하면 서비스가 시작된 후에 CCMSetup.exe가 종료되고 설치 세부 정보를 올바르게 보고하지 않을 수 있습니다.   
...
클라이언트 버전이 이미 설치되어 있는 경우 이 속성은 클라이언트 설치를 중지하도록 지정합니다.  

예: `ccmsetup.exe /logon`  

### <a name="forcereboot"></a>/forcereboot

 설치를 완료하기 위해 필요한 경우 CCMSetup이 클라이언트 컴퓨터를 강제로 다시 시작하도록 지정합니다. 이 속성이 지정되지 않는 경우 다시 시작이 필요한 때 CCMSetup이 종료합니다. 다음 수동으로 다시 시작 후 계속합니다.  
...
### <a name="usepkicert"></a>/UsePKICert
 ...
 이 옵션을 지정하면 클라이언트 인증이 포함된 PKI 인증서를 사용할 수 있는 경우 클라이언트에서 이 인증서를 사용합니다. 클라이언트가 유효한 인증서를 찾을 수 없는 경우 자체 서명된 인증서가 있는 HTTP 연결을 사용 합니다. 이 속성을 사용하지 않는 경우 이 동작은 동일합니다.

> [!NOTE]  
>  몇 가지 시나리오에서는 클라이언트를 설치할 때 이 속성을 지정할 필요가 없고, 클라이언트 인증서를 계속 사용할 수 있습니다. 이러한 시나리오에는 클라이언트 강제 및 소프트웨어 업데이트 지점 기반 클라이언트 설치를 사용하여 클라이언트를 설치하는 경우가 포함됩니다. 그러나 수동으로 클라이언트를 설치할 때는 언제나 이 속성을 지정해야 하고 **/mp** 속성을 사용하여 HTTPS 클라이언트 연결만 허용하도록 구성된 관리 지점을 지정해야 합니다. 또한 인터넷 전용 통신을 위한 클라이언트를 설치할 때 이 속성을 지정해야 합니다. CCMALWAYSINF=1 속성을 인터넷 기반 관리 지점 및 사이트 코드에 대한 속성과 함께 사용합니다. 인터넷 기반 클라이언트 관리에 대한 자세한 내용은 [인터넷 또는 신뢰할 수 없는 포리스트에서의 클라이언트 통신에 대한 고려 사항](../../plan-design/hierarchy/communications-between-endpoints.md#BKMK_clientspan)을 참조하세요.  
...
### <a name="skipprereqltfilename"></a>/skipprereq:&lt;파일 이름\>
 ...
 Configuration Manager 클라이언트를 설치할 때 CCMSetup.exe가 지정된 필수 구성 요소 프로그램을 설치하지 않도록 지정합니다. 이 속성에는 1 초과 값을 입력할 수 있습니다. 각 값을 구분하려면 세미콜론(;)을 사용합니다.  
...
> [!NOTE]  
>  **/ExcludeFeatures** 속성에 지원되는 값은 **ClientUI** 뿐입니다.  
...
### <a name="ccmalwaysinf"></a>CCMALWAYSINF
 ...
 이 속성은 인터넷 기반 관리 지점의 FQDN을 지정하는 CCMHOSTNAME과 함께 사용합니다. 또한 CCMSetup 속성 /UsePKICert 및 사이트 코드와 함께 사용합니다.  
