...
- **설명**: 소프트웨어 업데이트로 수정 또는 개선되는 조건에 대해 간략히 설명합니다.  
...
기본적으로 Configuration Manager 소프트웨어 업데이트의 서명을 확인할 때 CRL(인증서 해지 목록)은 확인되지 않습니다. 인증서가 사용될 때마다 CRL을 확인하면 해지된 인증서를 사용할 경우보다는 보안성이 향상되지만, 연결 지연이 발생할 수 있고 CRL 확인을 수행하는 컴퓨터에서 처리 부하가 증가합니다.  
