...
description: Configuration Manager에서 Wake On LAN 설정을 선택합니다.
ms.date: 08/07/2019
...
ms.locfileid: "75825169"
---
# <a name="how-to-configure-wake-on-lan-in-configuration-manager"></a>Configuration Manager에서 Wake on LAN을 구성하는 방법

*적용 대상: Configuration Manager(현재 분기)*

컴퓨터를 절전 모드에서 해제하려는 경우 Configuration Manager에 대해 Wake on LAN 설정을 지정합니다.
...
소프트웨어 업데이트, 애플리케이션, 작업 순서, 프로그램과 같은 필수 소프트웨어를 설치하기 위한 컴퓨터의 절전 모드를 해제하려는 경우 Configuration Manager에 대한 Wake on LAN 설정을 지정합니다.
...
> [!WARNING]
> 네트워크 서비스의 예상치 못한 중단을 방지하려면 먼저 격리된 전형적인 네트워크 인프라에서 절전 모드 해제 프록시를 평가합니다. 그런 다음 사용자 지정 클라이언트 설정을 사용하여 여러 서브넷의 선택한 컴퓨터 그룹으로 테스트를 확장합니다. 절전 모드 해제 프록시의 작동 원리에 대한 자세한 내용은 [클라이언트를 절전 모드에서 해제하는 방식 계획](../../../core/clients/deploy/plan/plan-wake-up-clients.md) 섹션을 참조하세요.
...
3. **Wake on LAN** 탭을 클릭하고 이 사이트에 필요한 옵션을 구성합니다. 절전 모드 해제 프록시를 지원하려면 **절전 모드 해제 패킷만 사용** 및 **유니캐스트**를 선택해야 합니다. 자세한 내용은 [클라이언트를 절전 모드에서 해제하는 방식 계획](../../../core/clients/deploy/plan/plan-wake-up-clients.md)을 참조하세요.
