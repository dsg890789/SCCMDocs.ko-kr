...
|프로세스|참조|  
|-------------|---------------|  
|ConfigMgr 관리자는 다음 요구 사항을 갖는 자동 배포 규칙을 만듭니다.<br /><br /><ol><li>ConfigMgr 관리자는 **일반** 탭에서 다음을 구성 합니다.<br /> <ul><li>이름으로 **Monthly Security Updates**를 지정합니다.</li><li>제한된 클라이언트가 포함된 테스트 컬렉션을 선택합니다.</li><li>**새 소프트웨어 업데이트 그룹 만들기**를 선택합니다.</li><li>**이 규칙을 실행한 후 배포 사용**이 선택되지 않았는지 확인합니다.</li></ul></li><li>**배포 설정** 탭에서 ConfigMgr 관리자는 기본 설정을 선택 합니다.</li><li>**소프트웨어 업데이트** 페이지에서 ConfigMgr 관리자는 다음 속성 필터와 검색 조건을 구성합니다.<br /><ul><li>릴리스 또는 수정 날짜 **지난 1개월**</li><li>업데이트 분류 **보안 업데이트**</li></ul></li><li>**평가** 페이지에서 ConfigMgr 관리자는 매**월** **두 번째 목요일**의 일정으로 규칙을 실행하도록 설정합니다.  또한 ConfigMgr 관리자는 동기화 일정을 매**월** **두 번째 수요일**에 실행하도록 설정했는지 확인합니다.</li><li>배포 일정, 사용자 환경, 경고, 다운로드 설정 페이지에서 기본 설정을 사용합니다.</li><li>**배포 패키지** 페이지에서 ConfigMgr 관리자는 새 배포 패키지를 지정 합니다.</li><li>다운로드 위치 및 언어 선택 페이지에서 기본 설정을 사용합니다.</li></ol>|[소프트웨어 업데이트 자동 배포](automatically-deploy-software-updates.md)|  

##  <a name="BKMK_Step3"></a> 3단계: 소프트웨어 업데이트 배포 준비 확인  
 ConfigMgr 관리자는 매월 두 번째 목요일에 소프트웨어 업데이트의 배포가 준비되었는지 확인합니다. 관리자는 다음 단계를 수행합니다.  

|프로세스|참조|  
|-------------|---------------|  
|ConfigMgr 관리자는 소프트웨어 업데이트 동기화가 성공적으로 완료되었는지 확인합니다.|[소프트웨어 업데이트 동기화 상태](monitor-software-updates.md#BKMK_SUSyncStatus)|  

##  <a name="BKMK_Step4"></a> 4단계: 소프트웨어 업데이트 그룹 배포  
 ConfigMgr 관리자는 소프트웨어 업데이트의 배포 준비가 완료 된 것을 확인 한 후 소프트웨어 업데이트를 배포 합니다. 관리자는 다음 표에 나온 단계를 수행합니다.  

|프로세스|참조|  
|-------------|---------------|  
|ConfigMgr 관리자는 새 소프트웨어 업데이트 그룹에 대해 두 개의 테스트 배포를 만듭니다. 그런 다음 각 배포에 대해 다음 환경을 고려합니다.<br /><br /> **워크스테이션 테스트 배포**: ConfigMgr 관리자는 워크스테이션 테스트 배포에 대해 다음 사항을 고려합니다.<br /><br /><ul><li>배포를 확인하기 위해 워크스테이션 클라이언트의 하위 집합이 포함된 배포 컬렉션을 지정합니다.</li><li>현재 환경의 워크스테이션 클라이언트에 적합한 배포 설정을 구성합니다.</li></ul><br />**서버 테스트 배포**: ConfigMgr 관리자는 서버 테스트 배포에 대해 다음 사항을 고려합니다.<br /><br /><ul><li>배포를 확인하기 위해 서버 클라이언트의 하위 집합이 포함된 배포 컬렉션을 지정합니다.</li><li>현재 환경의 서버 클라이언트에 적합한 배포 설정을 구성합니다.</li></ul>|[소프트웨어 업데이트 배포](deploy-software-updates.md)|  
