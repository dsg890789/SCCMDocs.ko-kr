...
1.  Technical Preview에 포함된 구성 스크립트를 사용하여 다음과 같이 회사 포털 배포 및 구성을 준비합니다.  
    ...
    1.  같은 폴더에 있는 **CompanyPortalOnPremisesMDM.appx**를 사용하여 Windows 앱 패키지 배포 유형으로 Configuration Manager 애플리케이션을 만듭니다.  

    2.  회사 포털을 구성하는 구성 항목 및 구성 기준을 만듭니다.  

    3.  구성 기준과 애플리케이션을 모두 배포하고 모든 배포 지점에 애플리케이션을 추가합니다.  
