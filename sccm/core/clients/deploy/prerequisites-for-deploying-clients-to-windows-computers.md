...
Windows 장치에 Configuration Manager 클라이언트를 설치하는 경우 필수 조건을 확인하려면 다음 정보를 참조하세요.  
...
클라이언트 강제를 사용하려면 다음 보안 권한이 필요합니다.  
   ...
   -   클라이언트 강제 설치 계정을 구성하려면: **사이트** 개체에 대한 **수정** 및 **읽기** 권한.  

   -   클라이언트 강제를 사용하여 클라이언트를 컬렉션, 장치 및 쿼리에 설치하려면: **컬렉션** 개체에 대한 **리소스 수정** 및 **읽기** 권한.  
...
#### <a name="microsoft-intune-mdm-installation"></a>Microsoft Intune MDM 설치
 ...
 - 인터넷 기반이 아니더라도 장치에 인터넷 액세스가 필요합니다.  
...
-   모바일 장치의 등록을 구성하려면 다음 보안 권한을 보유하고 있어야 합니다.  
     ...
     **운영 관리자** 기본 보안 역할에는 모바일 장치를 관리하는 데 필요한 권한이 포함됩니다.  
...
프록시 웹 서버를 사용하는 경우에는 SSL 터널링에 대해 구성되어야 합니다. 모바일 장치에서는 SSL 브리징이 지원되지 않습니다.  
